...
def _w_schedule(live, conn, cfg, args, origin):
    rule, log = live.set_schedule(
        conn, cfg, args.get("which", ""), origin=origin, cadence=args.get("cadence"),
...
def _w_move_once(live, conn, cfg, args, origin):
    event, replaced = live.move_one_occurrence(
        conn, cfg, args.get("which", ""), origin=origin, to=args.get("to", ""),
...
def _w_todo(live, conn, cfg, args, origin):
    ...
    todo, verb = live.open_todo(conn, cfg, args.get("text", ""), origin=origin,
                                due=args.get("due"), remind=remind,
...
def _w_note(live, conn, cfg, args, origin):
...
def _w_alias(live, conn, cfg, args, origin):
...
class MemcalMemoryProvider(MemoryProvider):
    ...
    def get_tool_schemas(self) -> List[Dict[str, Any]]:
        ...
        return [OPEN, OPEN_PAGE, SEARCH_WIKI, OPEN_SOURCE, CONVERSATION, LIST_DAYS,
                LIST_MONTH, SEARCH_ARCHIVE, ACTIVITY, REFRESH,
    ...
    def _dispatch(self, conn, tool_name: str, args: Dict[str, Any]) -> str:
        ...
        if tool_name in _WRITE_TOOLS:
            ...
            with self._lock:
                try:
                    # The turn that caused this call, supplied rather than guessed: this
                    # provider archived it itself, moments ago, in `on_turn_start`.
                    # `live` now writes the evidence link and the completed-operation
                    # record in the same transaction as the change, so the two can no
                    # longer come apart the way a post-hoc stamp could.
                    origin = live.Origin.of(
                        "hermes",
                        [self._turn_archive_id] if self._turn_archive_id else [],
                        session=self._session_id)
                    payload, refs = _WRITE_TOOLS[tool_name](
                        live, conn, self._cfg, args, origin)
                    # Wiki and alias writes do not go through a typed store, so they
                    # still need their own stamp.
                    for kind, ref, verb in refs:
                        ...
                        trace.stamp(
                            ...
                            stage="live", archive_ids=list(origin.archive_ids))
                    conn.commit()
