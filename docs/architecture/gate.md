...
in a 1:1 thread, a full-channel source (iMessage), or your own commitment or
directive. Most chatter fails everything and is archived for search at zero model
cost.

```bash
memcal gatecheck --channel groupme --limit 40   # what is passing and rejecting
...
or channel/thread) means never spending a model call there again; the archive rows
