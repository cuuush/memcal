...
def r1_beer(conn) -> tuple[bool, str]:
    """One beer plan on Sunday 2026-08-02 at Harborview Hall, with the people who said so.
    ...
    rows = _rows(conn, "SELECT key, date, time, title, location, participants FROM events "
                       "WHERE lower(title) LIKE '%beer%' OR lower(location) LIKE '%harborview%'")
    if not rows:
...
def r6_solstice(conn) -> tuple[bool, str]:
    """The Solstice festival keeps the calendar's dates: 2026-08-07 to 08-09.
    ...
    rows = _rows(conn, "SELECT key, date, until, source, origin FROM events "
                       "WHERE key LIKE 'ical-7ff0a4b829ec3391%' OR lower(title)='solstice'")
    if not rows:
        return False, "MISSING — the Solstice row is gone (check bench_reset)"
...
def r8_solstice_confirmed(conn) -> tuple[bool, str]:
    """Solstice is confirmed, not "maybe"."""
    rows = _rows(conn, "SELECT key, date, status, participants FROM events "
                       "WHERE lower(title) LIKE '%element%' AND date >= '2026-08-01'")
    if not rows:
        return False, "MISSING — no upcoming Solstice row"
...
def r9_solstice_swap(conn) -> tuple[bool, str]:
    """Morgan is no longer going to Solstice; their brother is going instead."""
...
def p3_festivals(conn) -> tuple[bool, str]:
    """Solstice and Moonrise are two festivals, not one.
    ...
    rows = _rows(conn, "SELECT key, date, title FROM events WHERE lower(title) LIKE '%element%'"
                       " OR lower(title) LIKE '%moonrise%' OR lower(title) LIKE '%forest%'")
...
def p8_no_bookkeeping_in_notes(conn) -> tuple[bool, str]:
    ...
    The brief carried "Beer garden · **2 sources mention this**" and "Solstice · **Nolan
...
def h1_same_day_dupes(conn) -> tuple[bool, str]:
    ...
    # of Solstice back in the report. The key prefix is minted by the feed and nothing
...
CHECKS = [
    ...
    ("recall",     "R6 solstice keeps ical dates",   r6_solstice),
    ("recall",     "R7 montana trip extracted",     r7_montana),
    ("recall",     "R8 solstice confirmed",          r8_solstice_confirmed),
    ("recall",     "R9 solstice: reese dropped",    r9_solstice_swap),
    ("precision",  "P1 partiful tag ≠ same event",   p1_partiful_tag),
    ("precision",  "P2 CTF repeat stays two",        p2_repeat_series),
    ("precision",  "P3 solstice ≠ moonrise",  p3_festivals),
