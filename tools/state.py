...
print("by channel:", [(r["channel"], r["n"], r["gated"])
                     for r in q("SELECT channel, count(*) n, sum(gated) gated FROM archive"
...
print("\n--- gated-but-never-spooled, by channel")
for r in q("""SELECT channel, count(*) n, min(ts) lo, max(ts) hi FROM archive a
               WHERE gated AND id NOT IN (SELECT archive_id FROM spool)
               GROUP BY 1 ORDER BY 2 DESC"""):
    print(f"  {r['n']:5}  {str(r['lo'])[:10]} -> {str(r['hi'])[:10]}  {r['channel']}")

print("\n--- threads seen, by channel (top 30)")
for r in q("""SELECT channel, thread, count(*) n, sum(gated) g, max(ts) hi
                FROM archive GROUP BY 1,2 ORDER BY n DESC LIMIT 30"""):
    print(f"  {r['n']:5} gated={r['g'] or 0:5}  {str(r['hi'])[:10]}  "
          f"{r['channel']}/{str(r['thread'])[:50]}")

print("\nunresolved:", q("SELECT count(*) c FROM unresolved")[0]["c"],
      [(r["channel"], r["n"]) for r in q("SELECT channel, count(*) n FROM unresolved"
                                        " GROUP BY 1 ORDER BY 2 DESC")])
