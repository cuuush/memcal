...
whether it is right is to read what it did. This prints both sides per channel, with
samples, and then the two questions that actually matter:
  ...
  * of the chat it dropped, how much was an answer to something it kept?
    ...
    python3 tools/audit_gate.py email        # one channel, more samples
"""
...
print("=" * 78)
for row in q("SELECT channel, count(*) n, sum(gated) g FROM archive GROUP BY 1 ORDER BY 2 DESC"):
    if only and row["channel"] != only:
        continue
    share = 100 * (row["g"] or 0) / row["n"]
    print(f"\n{row['channel']}: {row['n']} items, {row['g']} picked up ({share:.0f}%)")
    for r in q("""SELECT gate_reason reason, gated, count(*) n FROM archive
                   WHERE channel = ? GROUP BY 1,2 ORDER BY n DESC LIMIT 14""", row["channel"]):
...
if not only or only == "email":
    ...
    rows = q("""SELECT a.handle addr, count(*) n, max(a.ts) last,
                 ...
                 WHERE a.channel='email' AND a.from_me=0 AND a.gated=0
    ...
    for r in human:
        subj = q("""SELECT json_extract(meta,'$.subject') s FROM archive
                     WHERE channel='email' AND handle=? ORDER BY ts DESC LIMIT 1""",
                 r["addr"])
    ...
    for r in q("""SELECT a.handle addr, count(*) n, max(a.gate_reason) why
                    FROM archive a WHERE a.channel='email' AND a.from_me=0 AND a.gated=1
...
for channel in ("groupme", "whatsapp", "imessage"):
    if only and channel != only:
        continue
    dropped = q("""SELECT count(*) n FROM archive WHERE channel=? AND gated=0""", channel)[0]["n"]
    if not dropped:
        continue
    near = q("""SELECT count(*) n FROM archive d
                 WHERE d.channel=? AND d.gated=0 AND EXISTS (
                   SELECT 1 FROM archive k
                    WHERE k.channel=d.channel AND k.thread=d.thread AND k.gated=1
                      AND abs(julianday(k.ts) - julianday(d.ts)) <= 0.0833)""", channel)[0]["n"]
    day = q("""SELECT count(*) n FROM archive d
                WHERE d.channel=? AND d.gated=0 AND EXISTS (
                  SELECT 1 FROM archive k
                   WHERE k.channel=d.channel AND k.thread=d.thread AND k.gated=1
                     AND abs(julianday(k.ts) - julianday(d.ts)) <= 1.5)""", channel)[0]["n"]
    print(f"\n{channel}: {dropped} dropped · {near} within 2h of a kept line "
    ...
    for r in q("""SELECT d.text, d.ts, d.thread FROM archive d
                   ...
                   ORDER BY random() LIMIT 12""", channel):
...
counts = Counter()
for r in q("""SELECT channel, text FROM archive WHERE gated=0 AND channel != 'email'"""):
    if AGREE.match((r["text"] or "").strip()):
        counts[r["channel"]] += 1
for channel, n in counts.most_common():
    total = q("SELECT count(*) n FROM archive WHERE gated=0 AND channel=?", channel)[0]["n"]
    print(f"  {channel:9} {n:5} of {total} dropped lines are a bare yes/no ({100*n/total:.0f}%)")
if not counts:
    print("  (none)")
