...
from memcal import archive, brief, config, db                             # noqa: E402
...
from tests.scenarios import collision, expect, integration, load, probes  # noqa: E402
...
def run_layer(layer: str, home: Path, args) -> dict:
    ...
    if getattr(args, "provider", None):
        # Every stage, not just propose: a run that reads bundles on one backend and
        # arbitrates merges on another is measuring neither of them.
        cfg.llm_provider = args.provider
    if args.model:
        cfg.propose_model = args.model
        cfg.sweep_model = args.model
        cfg.match_model = args.model
...
def report(run: dict) -> str:
    ...
    if any(row.get("category") for row in run["results"]):
        lines.append(collision.category_report(run["results"]))
    if run["errors"]:
        lines.append("\nerrors:")
...
def probe_run(name: str, home: Path, case: str | None = None, *,
              layer: str = "apply", variants: int = 1,
    ...
    if name == "contract":
        ...
        # `case` limits execution here rather than only grading: one family is one
        # scenario's worth of work, which is the whole point of a selector on a suite
        # that runs a store per variant.
        rows = collision.collision_checks(
            home, case, layer=layer, variants=variants, settings=settings)
        case = None
    else:
...
def main() -> None:
    ...
    parser.add_argument("--provider", default=None,
                        choices=sorted(set(_llm.PROVIDER_DEFAULT_MODELS)),
                        help="override the LLM backend for this run. Without --model, "
                             "the provider's own default model comes with it — an "
                             "OpenRouter id handed to a CLI backend names nothing")
    ...
    parser.add_argument(
        "--suite", default="all",
        help="comma-separated: core,contract,boundaries,clock,schedule,collection,"
             "hermes,collision (default: all)")
    ...
    parser.add_argument("--variants", type=int, default=None, metavar="N",
                        help="collision suite: how many retellings of each scenario to "
                             "run beyond the plain one — the same story said differently, "
                             "with unrelated chatter mixed in, delivered twice, or "
                             "arriving out of order. Reproducible from the seed. "
                             "Default 1, or 0 with --layer model, where each one costs "
                             "real model calls")
    ...
    if args.variants is None:
        # A retelling is cheap deterministically and is a paid model run at the model
        # layer, so asking for the model layer must not quietly multiply the bill. Say
        # `--variants N` there and mean it.
        args.variants = 0 if args.layer == "model" else 1

    # Which backend this run measures. `config.load` resolves the default model from the
    # provider it read, so overriding the provider here without also re-resolving leaves
    # an OpenRouter id pointed at a CLI backend, which names nothing on that command.
    provider = (args.provider
                or str(getattr(config.load(), "llm_provider", _llm.DEFAULT_PROVIDER)))
    if args.provider and not args.model:
        args.model = _llm.PROVIDER_DEFAULT_MODELS.get(args.provider)
    ...
    # rather than warn.
    #
    # None of which is true of a CLI backend. `claude-code`, `codex` and `antigravity`
    # each talk to one vendor over one authenticated command — there is no routing to be
    # unpinned, and their native names are deliberately absent from `ENDPOINTS`
    # (Antigravity serves Gemini, Claude and open-weight models from one command, so no
    # single vendor prefix would find the right row). Refusing them here made three of
    # the four supported providers unreachable from this tool.
    if args.model and provider == "openrouter" and args.model not in _llm.ENDPOINTS:
        parser.error(
    ...
    if args.model and provider != "openrouter" and not _llm.ENDPOINTS.get(args.model):
        # Not a refusal, but not silent either: without a spec the run uses the
        # endpoint defaults, and `--effort` is the knob that puts a budget back.
        spec = _llm._spec_for(provider, args.model)
        if not spec.reasoning_effort and not args.effort:
            print(f"{YELLOW}note:{OFF} {provider} has no tuned reasoning budget for "
                  f"{args.model}; it runs on the backend's own default. Pass --effort "
                  f"to pin one.")
    ...
    if not args.dry_run:
        for name in suites:
            ...
            run = probe_run(name, base_home / f"{name}-probe", args.case,
                            layer=("model" if args.layer == "model" else "apply"),
                            variants=args.variants,
                            # The same knobs the core suite is run under, so a model
                            # measurement names the deployment it measured.
                            settings={"llm_provider": args.provider,
                                      "propose_model": args.model,
                                      "sweep_model": args.model,
                                      "match_model": args.model,
                                      "bundle_format": args.format,
                                      "prompt_version": args.prompt_version,
                                      "reasoning_effort": args.effort,
                                      "pack_bundles": args.pack,
                                      "propose_stages": args.stages})
            print(report(run))
