...
class TestTrackedFixturesContainOnlyApprovedSyntheticIdentities(unittest.TestCase):
    ...
    APPROVED_CAST = {
        "me": ("+19175550117", "88000", "19175550117@s.whatsapp.net"),
...
class TestEverySourceMetaKeyWrittenIsReadSomewhere(unittest.TestCase):
    """A written `source.*` marker that nothing reads is a signal only in appearance.

    `freshness` sweeps `source.%.last_success`, so an unread key can look meaningful even
    though no behavior depends on it.
    """

    #: An f-string slot and a SQL `%` are the same unknown here: one unspelled segment.
    WILDCARD = "\x00"

    #: What a `source.*` key looks like inside a longer string, such as a LIKE clause.
    FRAGMENT = re.compile(r"source\.[A-Za-z0-9_.%\x00]+")

    def _text(self, node):
        """The key a string node spells, with any segment it does not spell wildcarded."""
        if isinstance(node, ast.Constant) and isinstance(node.value, str):
            return node.value
        if isinstance(node, ast.JoinedStr):
            return "".join(
                part.value if isinstance(part, ast.Constant) and isinstance(part.value, str)
                else self.WILDCARD for part in node.values)
        return None

    def _scan(self, tree):
        """Split one module's strings into keys `set_meta` writes and everything else."""
        written, prose = [], set()
        for node in ast.walk(tree):
            ...
            name = node.func.attr if isinstance(node.func, ast.Attribute) else getattr(
                node.func, "id", "")
            if name == "set_meta":
                written.append(node.args[1])
        return written, prose

    def test_no_source_marker_is_written_without_a_reader(self):
        written: dict[str, str] = {}
        readers: list[str] = []
        skip: set[int] = set()
        trees = []
        for path in sorted((ROOT / "memcal").rglob("*.py")):
            tree = ast.parse(path.read_text(encoding="utf-8"))
            keys, prose = self._scan(tree)
            skip |= prose
            for node in keys:
                skip.add(id(node))
                key = self._text(node)
                if key and key.startswith("source."):
                    written.setdefault(key, f"{path.relative_to(ROOT)}:{node.lineno}")
            trees.append(tree)
        for tree in trees:
            for node in ast.walk(tree):
                if id(node) in skip:
                    continue
                text = self._text(node) if isinstance(
                    node, (ast.Constant, ast.JoinedStr)) else None
                if text and "source." in text:
                    readers += self.FRAGMENT.findall(text.replace(self.WILDCARD, "%"))
        patterns = [re.compile(re.escape(fragment).replace("%", ".*"))
                    for fragment in readers]
        unread = sorted(f"{key.replace(self.WILDCARD, '{}')} ({where})"
                        for key, where in written.items()
                        if not any(p.fullmatch(key.replace(self.WILDCARD, "anysource"))
                                   for p in patterns))
        self.assertEqual(
            unread, [],
            "these source markers are written and never read; delete them or give them "
            "the reader they imply")


class TestEveryConfigKnobHasAReader(unittest.TestCase):
    """A setting that reads as configuration and is not costs a reading every time
    somebody tunes. `verify`, `verify_budget` and `pack_cross_reference` were each
    documented, each wired into the env table, and each read by nothing — so
    `MEMCAL_VERIFY_BUDGET=8` set an attribute no code consulted, capping a feature that
    did not exist. `match_model` was worse: read once, to print itself in `doctor`."""

    #: Attributes that are the config layer's own plumbing rather than knobs.
    INTERNAL = {"home"}

    def _fields(self) -> list[str]:
        tree = ast.parse((ROOT / "memcal" / "config.py").read_text(encoding="utf-8"))
    ...
    def _readers(self) -> set[str]:
        """Every identifier used outside `config.py`, as identifiers rather than text.

        Read through `ast`, so a knob named in prose does not count as a reader: `verify`
        appears in a comment in `llm.py` and inside a regex in `todos.py`, and a textual
        scan calls it consumed on the strength of those. That is the same reasoning
        `TestEverySourceMetaKeyWrittenIsReadSomewhere` had to make about docstrings.
        """
        used: set[str] = set()
        for folder in ("memcal", "tools", "tests"):
            for path in sorted((ROOT / folder).rglob("*.py")):
                ...
                for node in ast.walk(ast.parse(path.read_text(encoding="utf-8"))):
                    if isinstance(node, ast.Attribute):
                        ...
                        # `getattr(cfg, "cold_start_waves", 4)` is a reader. The name is
                        # a string here and an identifier everywhere else.
                        used.add(str(node.args[1].value))
        return used

    def test_there_are_knobs_to_check(self):
        """`Config` restructured and this reports nothing at all."""
        self.assertGreater(len(self._fields()), 20)

    def test_no_setting_is_read_by_nothing_outside_config(self):
        readers = self._readers()
        orphans = [name for name in self._fields()
                   if name not in self.INTERNAL and name not in readers]
        self.assertEqual(orphans, [],
                         "delete these or give them a consumer; a documented knob that "
                         "changes nothing is worse than an absent one")

    def test_a_knob_printed_but_never_obeyed_is_still_reported(self):
        """The `match_model` shape, which a plain grep calls covered: `doctor` printed
        it, so the name appeared, while every Merge call still passed
        `propose_model`. Naming a setting in a model call is the reader that counts."""
        models = [name for name in self._fields() if name.endswith("_model")]
        self.assertTrue(models)
        calls = "\n".join(
            (ROOT / "memcal" / "dream" / part).read_text(encoding="utf-8")
            for part in ("propose.py", "merge.py", "sweep.py"))
        # `assertTrue` rather than `assertIn`: the haystack is three whole modules and
        # `assertIn` prints it.
        for name in models:
            self.assertTrue(f"cfg.{name}" in calls,
                            f"{name} selects no model; the stage that should use it "
                            f"passes another")
