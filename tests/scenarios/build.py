...
#: WhatsApp group JIDs.
#: Every WhatsApp thread and the JID its session carries. A group ends `@g.us`; a DM
#: carries the partner's own JID. Explicit rather than derived, so a thread nobody has
#: declared raises here instead of being silently written as a group — that loud failure
#: is the one good property the old map had, and it is kept.
WA_JIDS = {"dinner thu": "120363011111111111@g.us",
           "morgan family": "120363022222222222@g.us",
           "doggo park": "120363033333333333@g.us",
           "Rae": "19175550014@s.whatsapp.net"}

#: A group whose session type is unset exercises JID-based group detection.
WA_UNTYPED_SESSIONS = frozenset({"doggo park"})
...
def build_whatsapp(rows: list[dict]) -> int:
    ...
    Apple epoch (2001-01-01) timestamps, and sender linkage through ZWAGROUPMEMBER —
    which a DM does not have: there the sender is the chat itself, which is the branch
    of `ingest` that reads the address-book partner name.
    """
    path = OUT / "whatsapp"
    path.mkdir(parents=True, exist_ok=True)
        ...
        for row in rows:
            ...
            if thread not in sessions:
                ...
                conn.execute(
                    ...
                    " VALUES(?,?,?,?)",
                    (sessions[thread], jid, thread,
                     0 if thread in WA_UNTYPED_SESSIONS else int(group)))
            member_pk = None
            if group and row["who"] != "me":
