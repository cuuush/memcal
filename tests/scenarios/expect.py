...
class Ctx:
    ...
    def spool_priority(self, pattern: str) -> str:
        """`normal`, `low`, or why there is no answer. Relevance ranks; it excludes nothing."""
        rx = re.compile(pattern, re.I)
        for row in self.conn.execute(
                """SELECT a.text, coalesce(s.priority, 'normal') AS priority
                     FROM archive a JOIN spool s ON s.archive_id = a.id"""):
...
def publishes_location(pattern, wanted, **kw):
    ...
    calendar; tests never enable external calendar writes.
    """
...
def populated_archive_but_no_row(source_pattern, event_pattern, **kw):
    """The source pass ran, but policy deliberately kept one item out of typed state."""
    source_rx = re.compile(source_pattern, re.IGNORECASE)

    def run(ctx):
        source = next((row for row in ctx.conn.execute("SELECT text FROM archive")
                       if source_rx.search(str(row["text"] or ""))), None)
        rows = ctx.rows(event_pattern, **kw)
        ok = source is not None and not rows
        if source is None:
            return False, f"control /{source_pattern}/ was never archived"
        return ok, ("calendar source populated; no Memcal row"
                    if not rows else f"calendar source populated; {len(rows)} unwanted row(s)")
...
#: groupme:128934125's messages: What date is the BGC gathering", "+100000000000200
...
CHECKS: list[Check] = [
    ...
    Check("d1.generic-party-no-roster", "35 generic group event", 1,
          lambda c: (lambda e: (
              ...
              f"participants = {e.participants if e else '(no row)'}"))(
                  c.one(r"neon garden"))),
    Check("d1.solstice-one-row", "36 enrich existing group event", 1,
          count_rows(r"solstice", 1)),
    Check("d1.solstice-range", "36 enrich existing group event", 1,
          field_is(r"solstice", "until", "2026-08-09")),
    Check("d1.solstice-location", "36 enrich existing group event", 1,
          field_is(r"solstice", "location", "~rivertown")),
    Check("d1.solstice-attendees-not-roster", "36 enrich existing group event", 1,
          lambda c: (lambda e: (
              bool(e) and e.participants == ["Alex Rivera"],
              f"participants = {e.participants if e else '(no row)'}"))(
                  c.one(r"solstice"))),
    Check("d1.solstice-source", "36 enrich existing group event", 1,
          source_says(r"solstice", r"Friday August 7 through Sunday August 9")),
    ...
    # The wrong receipt and the marketing mail are read — quietly. What they must not do
    # is settle anything: the Batman row and the Superman to-do are the assertions that
    # matter, and they are checked on their own below. Excluding the messages outright
    # was never what protected those; it just also lost the mail.
    Check("gate.amc-wrong-arrives-quietly", "37 ticket lifecycle", 2,
          lambda c: (c.spool_priority(r"Batman Returns") == "low",
                     f"wrong receipt: {c.gate_reason(r'Batman Returns')} "
                     f"priority={c.spool_priority(r'Batman Returns')}")),
    Check("gate.amc-marketing-arrives-quietly", "37 ticket lifecycle", 2,
          lambda c: (c.spool_priority(r"Superman tickets are on sale") == "low",
                     f"marketing: {c.gate_reason(r'Superman tickets are on sale')} "
                     f"priority={c.spool_priority(r'Superman tickets are on sale')}")),
    ...
    Check("pages.user-facts-use-a-real-name", "17 wiki hygiene", 2,
          lambda c: ("casey" in c.pages() and "me" not in c.pages(),
                     f"pages={c.pages()}")),
    ...
    Check("gate.bulk-is-quiet-not-absent", "gate", 2,
          lambda c: (c.spool_priority(r"AWS Summit") == "low",
                     f"aws newsletter: {c.gate_reason(r'AWS Summit')} "
                     f"priority={c.spool_priority(r'AWS Summit')}")),
    ...
    Check("state.only-declared-events", "32 closed-world inventory", 2,
          only_expected_events([
              ...
              (r"tutoring", WED_12),
              # The Drafthouse receipt reaches the store now that a transactional
              # sender's body is fetched before its relevance is decided, so its row is
              # a declared part of the final state rather than an invention.
              (r"dune|drafthouse", WED_12),
    ...
    Check("state.only-declared-aliases", "32 closed-world inventory", 2,
          only_expected_aliases({
              "katie": [r"Kat"],
              "u-and-me-calendar": [r"our cal", r"shared cal", r"u&me"],
          })),
    ...
    Check("solstice.breakfast-exists", "47 containment", 3,
          row_on(r"breakfast", SAT)),
    Check("solstice.breakfast-nested", "47 containment", 3,
          lambda c: (lambda child, parent: (
              ...
              f"festival id = {parent.id if parent else None}"))(
                  c.one(r"breakfast", on=SAT), c.one(r"solstice music festival"))),
    Check("solstice.breakfast-not-merged", "47 containment", 3,
          count_rows(r"solstice", 2)),
    Check("solstice.nested-in-brief", "47 containment", 3,
    ...
    Check("solstice.festival-keeps-its-span", "47 containment", 3,
          field_is(r"solstice music festival", "until", SUN)),
    ...
    # Graded on day 4, where the row is the *declined* invitation the brief keeps on
    # purpose, rather than on day 2, where it competes with every other dated row for
    # one of `_later_block`'s eight slots. Whether an unanswered invite two weeks out
    # wins a slot against the week's own traffic is a rendering budget, not this beat's
    # claim; `invite.declined-keeps-its-link` makes the same claim without the race.
    Check("invite.brief-names-the-link", "48 an invitation is actionable", 4,
    ...
    Check("jack30.still-one-row", "45 rescan must not undo judgement", 4,
          count_rows(r"jack's 30th", 1)),
    Check("feed.stays-reference-only", "45 rescan must not undo judgement", 4,
          populated_archive_but_no_row(r"dentist", r"statehood")),
