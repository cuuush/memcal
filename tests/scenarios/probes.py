...
def boundary_checks(home: Path) -> list[dict]:
    ...
    try:
        ...
        proton._handle_message(
            ...
            message("appt-1@example.com", "Appointment Tuesday",
                    "Sat, 13 Mar 2027 10:00:00 -0500"),
            "INBOX")
        proton._handle_message(
            conn, cfg, report, mailbox, 2,
            message("appt-2@example.com", "Re: Appointment Tuesday",
                    "Sat, 13 Mar 2027 10:05:00 -0500",
                    reply_to="appt-1@example.com"),
            "INBOX")
        proton._handle_message(
...
def clock_checks(home: Path) -> list[dict]:
    ...
    try:
        ...
        festival, _ = events.upsert(conn, {
            "title": "Solstice festival", "date": "2026-08-07", "until": "2026-08-09",
            "kind": "commitment", "status": "declined", "subject": "me"},
            written_by="ical")
        events.upsert(conn, {
            "title": "Breakfast at Solstice", "date": "2026-08-08",
            "kind": "commitment", "status": "confirmed", "subject": "me"},
            written_by="dream:day1")
        ...
        todos.ask(conn, "Which days is the Solstice festival running?",
        ...
        breakfast = conn.execute(
            "SELECT id FROM events WHERE title = 'Breakfast at Solstice'").fetchone()
        conn.execute("UPDATE questions SET about_event = ? WHERE key = 'q:festival'",
                     (breakfast["id"],))
        conn.commit()
        checks.append(result(
            "clock.stale-link-was-made", "question linking",
            linked_title("q:festival") == "Breakfast at Solstice",
        ...
        events.upsert(conn, {
            "key": festival.key, "title": "Solstice festival", "date": "2026-08-07",
        ...
        checks.append(result(
            ...
            linked_title("q:festival") == "Solstice festival",
        ...
        #
        # The user was at a festival in Rivertown, Vermont that weekend, and the store
...
def schedule_checks(home: Path) -> list[dict]:
    ...
    try:
        ...
        # delete an observation, so it becomes a question instead.
...
def hermes_checks(home: Path) -> list[dict]:
    ...
    # The plugin hot-reloads by dropping every `memcal.*` module out of `sys.modules`,
    # so anything importing memcal after this suite gets a *second* copy of it. The
    # modules this file bound at import keep the pinned test clock; the fresh copy does
    # not, and a suite running afterwards reads the wall clock through whichever half
    # it happens to reach. Three question-linking checks in `clock` went red that way,
    # scoring the run wrong with nothing saying so.
    kept_modules = {name: module for name, module in sys.modules.items()
