...
- **Must:** the to-do exists after day 1 and is **still open** after day 2, with a
  question raised about it. Closure is conversational, never inferred.
...
- **Must not:** the pass, reading the same sentence, re-open it or open a second one
  beside it. It is forbidden from closing a to-do on inference, and this
...
- **D1** a live Solstice Music Festival row already exists with only its Friday start.
  The rave chat names the same festival, says it runs through Sunday at Rivertown, and
...
- **Must:** the existing row is enriched rather than duplicated; `until` is Sunday,
  location is Rivertown, and Alex is the only stored participant.
...
- **D1** the Solstice festival runs Fri 7 – Sun 9 August.
- **D3** the rave chat settles breakfast at Solstice on the Saturday morning.
...
- Three live rows were all called "Solstice" and read as three unrelated plans on one
