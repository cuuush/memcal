...
def agent_actions(conn, cfg, day: int) -> list[str]:
    ...
    pinned = sk.DAYS[day - 1]
    for action in sk.ACTIONS:
        if action["day"] != day:
            continue
        # Each action carries the time of day it happened, and until now the runner
        # threw it away: every call in a day landed at whatever moment the loop reached
        # it, so `updated_at` could not order two tool calls against each other or
        # against the traffic between them. Write precedence is decided on exactly those
        # comparisons, which made the one ordering that matters unrepresentable here.
