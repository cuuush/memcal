...
from memcal import archive, brief, cli, dates, db, detail, events, gate, identity, live, mcp_server, pending, schedule, series, textclean, threads, todos, trace, web, web_server, wiki  # noqa: E402
...
class TestReportedMemoryFailures(Base):
    ...
    def test_a_reaction_is_rescued_when_the_topic_appears_after_it(self):
        ...
        self.assertEqual(row["entity"], "thread:groupme:BGC IRL BGN",
...
class TestAPartifulEventStillOnTheCalendarIsNotDeclined(Base):
    ...
    def test_an_identity_change_is_not_an_rsvp(self):
        db.set_today("2026-08-01")
        self.snapshot([self.item("solstice", "We're Going to Solstice!!!", 6,
                                 calendar_uid="")])
        db.set_today("2026-08-04")
        report = self.snapshot([self.item("solstice", "We're Going to Solstice!!!", 3,
                                          calendar_uid="Partiful")])
        rows = [r for r in events.window(self.conn, 0, 10)
                if r.title.startswith("We're Going to Solstice")]
...
class TestMemcalDoesNotReadItsOwnPublishedRowBackIn(Base):
    ...
    def test_a_publish_record_follows_its_row_to_a_new_key(self):
        ...
        # Move and re-key the row to reproduce the stored corruption.
...
class TestMemcalsCalendarIsNotLocal(Base):
    ...
    def test_a_store_that_does_not_publish_never_shells_out(self):
        """External calendar writes default to off."""
...
class TestThreeRowsCalledSolsticeWereThreeUnrelatedPlans(Base):
    ...
    def _weekend(self):
        festival, _ = events.upsert(self.conn, {
            "title": "Solstice Music Festival", "date": self.d(4), "until": self.d(6),
            "kind": "commitment", "status": "confirmed",
            "participants": ["Alex Rivera"]})
        breakfast, _ = events.upsert(self.conn, {
            "title": "Breakfast at Solstice", "date": self.d(5), "time": "09:00",
            "kind": "commitment", "status": "confirmed",
            "participants": ["Alex Rivera"]})
        # Two rows is the premise of every test below, and `upsert` deciding these are
        # one is the regression this class is named for — so the fixture states it.
        self.assertNotEqual(festival.id, breakfast.id,
                            "the weekend collapsed into one row")
    ...
    def test_a_row_inside_a_span_that_does_not_name_it_is_left_alone(self):
        """Falling on the same weekend is not evidence of containment."""
    ...
    def test_two_overlapping_spans_are_two_plans(self):
        ...
        trip, _ = events.upsert(self.conn, {
            "title": "Solstice road trip", "date": self.d(5), "until": self.d(7),
            "status": "confirmed"})
        self.assertNotEqual(festival.id, trip.id)
        events.link_contained(self.conn)
    ...
    def test_the_brief_nests_it(self):
        ...
        events.link_contained(self.conn)
        text = brief.render(self.conn, self.cfg)
        self.assertIn("Solstice Music Festival", text)
        self.assertRegex(text, r"↳ .*Breakfast at Solstice")
...
class TestAWordEveryTitleCarriesIsNotAName(Base):
    ...
    def test_a_common_word_does_not_nest(self):
        ...
        party, _ = events.upsert(self.conn, {
            ...
            "kind": "opportunity", "status": "mentioned"}, match=False)
        events.link_contained(self.conn)
...
class TestAWrongNestingWasPermanent(Base):
    ...
    it. Known-bad inferred nesting is repaired, not preserved."""
...
class TestOnlyACommitmentReachesTheRealCalendar(Base):
    """Nothing that writes outside this process may be enabled by accident.
...
class TestAQuestionLinkedOnOneGenericWord(Base):
    ...
    def test_an_occasion_noun_is_not_a_subject(self):
        events.upsert(self.conn, {"title": "Alumni meeting", "date": self.d(6),
                                  "status": "confirmed"})
        key = todos.ask(self.conn, "Is the BGC meeting still happening on Saturday?")
    ...
    def test_a_word_two_rows_share_no_longer_identifies_either(self):
        events.upsert(self.conn, {"title": "Solstice festival", "date": self.d(4),
                                  "status": "confirmed"})
        events.upsert(self.conn, {"title": "Breakfast at Solstice", "date": self.d(5),
                                  "status": "confirmed"})
        key = todos.ask(self.conn, "Who else is going to Solstice?")
        todos.relink_questions(self.conn)
...
class TestAStaleQuestionLinkWasNeverReScored(Base):
    ...
    question is stuck on "Breakfast at Solstice" because it linked while the festival
    ...
    def test_a_better_candidate_moves_the_link(self):
        festival, _ = events.upsert(self.conn, {
            ...
            "status": "declined"})
        breakfast, _ = events.upsert(self.conn, {
            "title": "Breakfast at Solstice", "date": self.d(5), "status": "confirmed"})
        key = todos.ask(self.conn, "Which days is the Solstice festival running?")
        self.conn.execute("UPDATE questions SET about_event = ? WHERE key = ?",
                          (breakfast.id, key))
        self.conn.commit()
        events.upsert(self.conn, {"key": festival.key, "title": "Solstice festival",
...
class TestAQuestionWasNeverClosedByItsAnswer(Base):
    ...
    def test_an_unlinked_obligation_is_not_a_question_and_survives(self):
        """A to-do with no occasion attached has no day to
...
class TestAQuestionWithNoEventLinkHadNoDayToDieWith(Base):
    ...
    def test_a_question_naming_no_day_waits_out_its_ttl(self):
        ...
        it has nothing to die with, so expiry must not invent one."""
        key = todos.ask(self.conn,
...
class TestAConfirmedRowReadCouldGo(Base):
    ...
    def test_a_settled_status_outranks_the_kind(self):
        row, _ = events.upsert(self.conn, {
            "title": "Solstice festival", "date": self.d(4), "kind": "opportunity",
            "status": "confirmed"})
...
class TestTheSweepAskedTheUserToDoMemcalsHomework(Base):
    ...
    three August 7 Solstice entries the same event…?" — which the sweep's own
    ...
    def test_deduplicating_the_calendar_is_not_a_question(self):
        self.assertFalse(todos.is_worth_asking(
            "Were the three August 7 Solstice entries the same event or three separate?"))
...
class TestASeriesHadNoScheduleToChange(Base):
    ...
    def test_a_series_with_no_cadence_refuses_to_project(self):
        ...
        It must not project onto an invented weekday."""
        rule, _ = series.upsert(self.conn, {
...
class TestOneWeekMovedIsNotTheCadenceMoving(Base):
    ...
    def test_a_projection_the_rule_no_longer_lands_on_is_deleted(self):
        """The withdrawal half, which nothing observed. Its sibling below asserts only
        that an *observation* survives, and `log.append` runs whether or not the row
        went — so disabling `events.delete` here left the suite green *and* the log
        still saying "withdrew". The row is the only witness."""
        db.set_today(date(2026, 8, 11))
        try:
            series.roll_forward(self.conn, slug="tutoring")
            self.assertIsNotNone(events.get(self.conn, "tutoring@2026-08-11"),
                                 "the fixture never projected anything to withdraw")
            series.upsert(self.conn, {"slug": "tutoring", "weekday": 3},
                          written_by="live")
            log = series.roll_forward(self.conn, slug="tutoring")
            dates = [r["date"] for r in self.conn.execute(
                "SELECT date FROM events WHERE series = 'tutoring' ORDER BY date")]
        finally:
            db.set_today(None)
        self.assertIsNone(events.get(self.conn, "tutoring@2026-08-11"))
        self.assertEqual(dates, ["2026-08-13"])          # the Thursday it moved to
        self.assertTrue(any("withdrew" in line for line in log), log)
...
class TestAStandingAppointmentReachedTheCalendarAsOneDatedCopy(Base):
    ...
    def test_publishing_stays_off_unless_a_calendar_is_named(self):
        """Nothing that writes outside this process defaults to on, and a
...
class TestARowBecameHappenedWithNothingSayingItDid(Base):
    """The nightly settling of past rows left no record of the change.

    `mark_past_happened` was a bare UPDATE over a tracked field, so a row could go from
    confirmed to happened overnight and its history would never mention a status.
    """
    ...
    def _row(self, title, day, status):
        event, _ = events.upsert(
            self.conn, {"title": title, "date": day, "status": status}, written_by="cli")
        return event

    def _statuses(self, event_id):
        return [(r["old_value"], r["new_value"], r["written_by"])
                for r in events.history(self.conn, event_id) if r["field"] == "status"]

    def test_the_status_it_left_behind_is_recorded(self):
        event = self._row("Dinner", self.d(-3), "confirmed")
        events.mark_past_happened(self.conn)
        self.assertEqual([("confirmed", "happened")],
                         [(old, new) for old, new, _by in self._statuses(event.id)])

    def test_a_tentative_row_records_its_own_old_value(self):
        """One history row per event, not one shared story about the pass."""
        maybe = self._row("Maybe drinks", self.d(-2), "tentative")
        sure = self._row("Standup", self.d(-2), "confirmed")
        events.mark_past_happened(self.conn)
        self.assertEqual([("tentative", "happened")],
                         [(old, new) for old, new, _by in self._statuses(maybe.id)])
        self.assertEqual([("confirmed", "happened")],
                         [(old, new) for old, new, _by in self._statuses(sure.id)])

    def test_the_change_names_the_pass_that_made_it(self):
        event = self._row("Dinner", self.d(-3), "confirmed")
        events.mark_past_happened(self.conn)
        written_by = self._statuses(event.id)[0][2]
        self.assertTrue(written_by.startswith("code:"),
                        f"an anonymous change is barely a record: {written_by!r}")

    def test_a_row_it_does_not_touch_gains_no_history(self):
        soon = self._row("Dinner", self.d(3), "confirmed")
        mentioned = self._row("Someone said something", self.d(-3), "mentioned")
        events.mark_past_happened(self.conn)
        self.assertEqual([], self._statuses(soon.id))
        self.assertEqual([], self._statuses(mentioned.id))

    def test_the_count_of_settled_rows_is_still_what_comes_back(self):
        """Callers count the pass by this number; recording history must not change it."""
        self._row("Dinner", self.d(-3), "confirmed")
        self._row("Standup", self.d(-2), "tentative")
        self._row("Later", self.d(3), "confirmed")
        self.assertEqual(2, events.mark_past_happened(self.conn))

    def test_a_second_pass_does_not_repeat_itself(self):
        event = self._row("Dinner", self.d(-3), "confirmed")
        events.mark_past_happened(self.conn)
        self.assertEqual(0, events.mark_past_happened(self.conn))
        self.assertEqual(1, len(self._statuses(event.id)))

    def test_a_calendar_rescan_can_no_longer_un_happen_a_past_row(self):
        """The consequence of recording the change, and worth pinning deliberately.

        `_claimed_by_another` reads a history row as a claim, so stamping the settling
        gives past rows a status claim they did not have. The calendar connector derives
        `status` on every scan with no new information, and could previously walk a past
        row back to confirmed; now it cannot. A decline is unaffected, because
        `partiful.reconcile_missing` marks nothing inferred.
        """
        event = self._row("Dinner", self.d(-3), "confirmed")
        events.mark_past_happened(self.conn)
        events.upsert(self.conn, {"key": event.key, "title": "Dinner",
                                  "date": self.d(-3), "status": "confirmed"},
                      written_by="ical", match=False, inferred=("status",))
        self.assertEqual(events.get_by_id(self.conn, event.id).status, "happened")
...
class TestMailIsIncludedByDefaultAndRankedNotDropped(Base):
    """Automatic relevance decides when mail is read, never whether it exists.

    The old behaviour was permanent and invisible: a sender filed under `archive` for
    carrying a List-Id had its body left unfetched forever, so the one thing that could
    ...
    def _message(self, message_id, subject, *, sender="news@shop.example",
                 when="Mon, 07 Sep 2026 10:00:00 -0400", reply_to="", extra=()):
        headers = [f"Message-ID: <{message_id}>", f"Date: {when}",
                   f"From: Sender <{sender}>", "To: Casey <casey@example.com>",
                   f"Subject: {subject}", *extra]
        if reply_to:
            headers += [f"In-Reply-To: <{reply_to}>", f"References: <{reply_to}>"]
        return email.message_from_string("\n".join(headers) + "\n\nbody")

    def _deliver(self, uid, message, mailbox):
        report = base.IngestReport.opened("email", self.cfg)
        proton._handle_message(self.conn, self.cfg, report, mailbox, uid, message,
                               "INBOX")
        return report

    def test_a_bulk_senders_body_is_fetched_and_kept(self):
        mailbox = self.Mailbox({1: "Your delivery window is Tuesday 9-11am."})
        self._deliver(1, self._message(
    ...
    def test_it_is_queued_quietly_rather_than_excluded(self):
        mailbox = self.Mailbox({1: "40% off everything."})
        self._deliver(1, self._message(
            "bulk-2@shop.example", "Sale ends tonight",
    ...
    def test_a_person_saying_no_still_keeps_mail_out_entirely(self):
        identity.set_sender(self.conn, "news@shop.example", "ignore", "don't care",
                            source="you")
        mailbox = self.Mailbox({1: "anything"})
        self._deliver(1, self._message("bulk-3@shop.example", "Anything"), mailbox)
        queued = self.conn.execute(
            "SELECT count(*) AS n FROM spool").fetchone()["n"]
        self.assertEqual(queued, 0)

    def test_a_blocked_senders_body_is_never_opened(self):
        """"Entirely" has to include the body, or the block is only a filing decision.

        Fetching for everything in scope made the automatic verdicts reviewable, which is
        the point — but it ran after the verdict and so ran for a blocked sender too,
        decrypting their mail into the archive and showing it in the queue.
        """
        identity.set_sender(self.conn, "news@shop.example", "ignore", "don't care",
                            source="you")
    ...
    def test_a_blocked_senders_mail_is_not_a_backfill_candidate(self):
        """Otherwise the repair pass walks straight around the exclusion."""
        identity.set_sender(self.conn, "news@shop.example", "ignore", "don't care",
                            source="you")
        self._deliver(1, self._message("bulk-5@shop.example", "Anything"),
                      self.Mailbox({1: "anything"}))
        self.assertEqual(proton.recoverable(self.conn), [])

    def test_a_failed_body_fetch_is_recorded_rather_than_looking_complete(self):
    ...
    def test_retrying_a_failed_fetch_updates_the_row_instead_of_duplicating_it(self):
        message = self._message("fail-2@shop.example", "Your order")
...
class TestTheQuietBacklogIsBoundedAndVisible(Base):
    """Low priority must mean "read later", not "read never"."""
    ...
    def test_a_quiet_backlog_cannot_crowd_out_the_days_own_traffic(self):
        self._queue(40, "low", entity="bulk")
        self._queue(10, "normal", entity="people")
        rows = archive.spool_pending(self.conn, limit=20, per_entity=100)
        priorities = [row["priority"] for row in rows]
        self.assertEqual(priorities.count("normal"), 10)
        self.assertTrue(priorities.count("low") > 0)

    def test_what_did_not_fit_is_still_waiting_and_says_so(self):
        self._queue(40, "low", entity="bulk")
        archive.spool_pending(self.conn, limit=5, per_entity=100)
        waiting = archive.backlog(self.conn)
        self.assertEqual(waiting["low"]["waiting"], 40)
        self.assertEqual(waiting["total"], 40)


class TestACancellationWithNoTargetIsKeptRatherThanInventedOrDropped(Base):
    """The two things that must not happen to "that's cancelled".

    Dropped, and a real statement about the user's week is gone. Written as a row, and the
    calendar acquires a cancelled plan nobody ever made. Held instead, with its evidence,
    and retried when the traffic that places it arrives.
    """
    ...
    def test_a_cancellation_naming_no_stored_row_writes_no_row(self):
        bundle = self._bundle("the thing on saturday is off")
        apply_stage.apply_diffs(self.conn, self.cfg, [(bundle, {
            "events": [{"title": "The thing", "date": "2026-09-19",
                        "status": "declined", "subject": "me"}],
            "todos": [], "wiki": [], "questions": []})], written_by="dream:nightly")
        self.assertEqual(
            self.conn.execute("SELECT count(*) AS n FROM events").fetchone()["n"], 0)
        self.assertEqual(len(pending.open_items(self.conn)), 1)

    def test_a_verified_identifier_licenses_the_write(self):
        """One of the two things that does: the event's own key, carried on the record."""
        db.set_today("2026-09-14T09:00:00")
        self.addCleanup(db.set_today, None)
        row, _ = events.upsert(self.conn, {"title": "Climbing gym", "date": "2026-09-19",
                                           "status": "confirmed"}, written_by="live")
        pending.note(self.conn, kind="cancellation",
                     observation="Climbing gym on 2026-09-19",
                     title="Climbing gym", date="2026-09-19", target_key=row.key,
                     decided_by="model:cited L4", observed_at="2026-09-15T18:00:00",
                     entity="person:Riley Morgan", commit=True)
        db.set_today("2026-09-16T09:00:00")
        pending.retry(self.conn)
        self.assertEqual(events.get(self.conn, row.key).status, "declined")
        self.assertEqual(pending.open_items(self.conn), [])

    def test_a_cited_decision_licenses_it_and_nothing_else_does(self):
        """The other: somebody said which row it was, and the record says who."""
        db.set_today("2026-09-14T09:00:00")
        self.addCleanup(db.set_today, None)
        row, _ = events.upsert(self.conn, {"title": "Climbing gym", "date": "2026-09-19",
                                           "status": "confirmed"}, written_by="live")
        pending.note(self.conn, kind="cancellation",
                     observation="climbing is off", observed_at="2026-09-15T18:00:00",
                     entity="person:Riley Morgan", commit=True)
        pending.retry(self.conn)
        self.assertEqual(events.get(self.conn, row.key).status, "confirmed")

        item = pending.open_items(self.conn)[0]
        self.assertTrue(pending.settle(self.conn, item["id"], row.key,
                                       decided_by="you:answered q:pending"))
        db.set_today("2026-09-16T09:00:00")
        pending.retry(self.conn)
        self.assertEqual(events.get(self.conn, row.key).status, "declined")

    def test_answering_the_question_is_what_closes_the_loop(self):
        """The whole point of asking, and nothing was reading the answer back.

        `_identified` needs `target_key`, `settle` is the only writer of it, and no
        production path called `settle` — so a cancellation was asked about once and then
        held open for good, however plainly the user answered.
        """
        db.set_today("2026-09-14T09:00:00")
        self.addCleanup(db.set_today, None)
        row, _ = events.upsert(self.conn, {"title": "Climbing gym", "date": "2026-09-19",
                                           "status": "confirmed"}, written_by="live")
        pending.note(self.conn, kind="cancellation", observation="the climbing gym is off",
                     observed_at="2026-09-15T18:00:00", entity="person:Riley Morgan",
                     commit=True)
        asked = []
        pending.retry(self.conn, ask=lambda text, key: asked.append(
            todos.ask(self.conn, text, key=key, written_by="dream")))
        self.assertTrue(asked)
        self.assertEqual(events.get(self.conn, row.key).status, "confirmed")

        item = pending.open_items(self.conn)[0]
        self.assertTrue(todos.answer(self.conn, f"q:pending:{item['id']}",
                                     "the climbing gym one"))
        db.set_today("2026-09-16T09:00:00")
        pending.retry(self.conn)
        self.assertEqual(events.get(self.conn, row.key).status, "declined")
        self.assertEqual(pending.open_items(self.conn), [])

    def test_answering_that_it_was_none_of_them_stops_the_asking(self):
        db.set_today("2026-09-14T09:00:00")
        self.addCleanup(db.set_today, None)
        row, _ = events.upsert(self.conn, {"title": "Climbing gym", "date": "2026-09-19",
                                           "status": "confirmed"}, written_by="live")
        pending.note(self.conn, kind="cancellation", observation="the climbing gym is off",
                     observed_at="2026-09-15T18:00:00", entity="person:Riley Morgan",
                     commit=True)
        pending.retry(self.conn, ask=lambda text, key: todos.ask(
            self.conn, text, key=key, written_by="dream"))
        item = pending.open_items(self.conn)[0]
        todos.answer(self.conn, f"q:pending:{item['id']}", "none of those")
        pending.retry(self.conn)
        self.assertEqual(events.get(self.conn, row.key).status, "confirmed")
        self.assertEqual(pending.open_items(self.conn), [])

    def test_another_appointment_at_the_same_provider_is_never_the_target(self):
        """The matcher that reunites a mention with a row is not an authorisation.

        `find_match` joins on a shared title, and a provider calls every appointment the
        same thing — so "your physio on the 18th is cancelled", with only a physio on the
        14th in the store, would have cancelled an appointment the user still has.
        """
        db.set_today("2026-09-12T09:00:00")
        self.addCleanup(db.set_today, None)
        kept, _ = events.upsert(
            self.conn, {"title": "Physio session", "date": "2026-09-14",
                        "time": "17:00", "location": "Riverton PT",
                        "status": "confirmed"}, written_by="live")
        pending.note(self.conn, kind="cancellation",
                     observation="Physio session on 2026-09-18",
                     title="Physio session", date="2026-09-18",
                     observed_at="2026-09-13T09:00:00",
                     entity="thread:email:clinic@riverton.example", commit=True)
        asked = []
        db.set_today("2026-09-13T23:30:00")
        pending.retry(self.conn, ask=lambda text, key: asked.append(text))
        self.assertEqual(events.get(self.conn, kept.key).status, "confirmed")
        self.assertEqual(kept.date, "2026-09-14")
        self.assertEqual(len(pending.open_items(self.conn)), 1)
        self.assertEqual(len(asked), 1)

    def test_a_matcher_hit_is_still_not_an_authorisation(self):
        """The rule holds on its own, not because the matcher happens to miss.

        Here `find_match` does return the row — same title, same day — and the
        observation still may not act on it, because a forgiving matcher is the wrong
        instrument for a decision with no undo.
        """
        db.set_today("2026-09-12T09:00:00")
        self.addCleanup(db.set_today, None)
        kept, _ = events.upsert(
        ...
        self.assertIsNotNone(
            events.find_match(self.conn, title="Physio session", on="2026-09-14"),
            "this case is only meaningful while the matcher does hit")
        pending.note(self.conn, kind="cancellation",
                     observation="Physio session on 2026-09-14",
                     title="Physio session", date="2026-09-14",
                     observed_at="2026-09-13T09:00:00",
                     entity="thread:email:clinic@riverton.example", commit=True)
        db.set_today("2026-09-13T23:30:00")
        pending.retry(self.conn)
    ...
    def test_a_row_that_merely_shares_the_day_is_never_cancelled(self):
        """The failure this guard exists for.

        Sharing a date is one of the edges that earns a row a *look*; it is not evidence
        that the row is the one meant. Being the only candidate shown is not the same as
        being identified, and there is no undo a person would ever find for a plan
        silently cancelled out from under them.
        """
        db.set_today("2026-09-14T09:00:00")
        self.addCleanup(db.set_today, None)
        events.upsert(self.conn, {"title": "Piano lesson", "date": "2026-09-15",
                                  "status": "confirmed"}, written_by="live")
        pending.note(self.conn, kind="cancellation",
                     observation="Dental cleaning on 2026-09-15",
                     title="Dental cleaning", date="2026-09-15",
                     entity="person:Riley Morgan", commit=True)
        asked = []
        db.set_today("2026-09-14T20:00:00")
        pending.retry(self.conn, ask=lambda text, key: asked.append(text))
        piano = events.get(self.conn, events.make_key("Piano lesson", "2026-09-15"))
        self.assertEqual(piano.status, "confirmed")
        self.assertEqual(len(pending.open_items(self.conn)), 1)
        self.assertEqual(len(asked), 1)

    def test_a_single_weak_candidate_is_held_rather_than_applied(self):
        """With nobody to ask, the observation waits. It does not pick the one on offer."""
        db.set_today("2026-09-14T09:00:00")
        self.addCleanup(db.set_today, None)
        events.upsert(self.conn, {"title": "Piano lesson", "date": "2026-09-15",
                                  "status": "confirmed"}, written_by="live")
    ...
    def test_an_ambiguous_one_asks_instead_of_choosing(self):
        ...
        pending.retry(self.conn, ask=lambda text, key: asked.append(text))
        self.assertEqual(len(asked), 1)
        self.assertIn("Which one", asked[0])
        self.assertEqual(len(pending.open_items(self.conn)), 1)


class TestACorrectionFromSomeoneNotOnTheRowIsStillShownTheRow(Base):
    """Nominations, not matches — but a candidate nobody shows the model is a duplicate.

    Every edge `amendable_groups` had was a link to a person already on the row or to a
    conversation that had already written it. A correction from a third party, on another
    channel, eleven days later, has neither, so the pass was shown nothing at all.
    """
    ...
    def setUp(self):
        ...
        events.upsert(self.conn, {"title": "Ramen dinner", "date": "2026-09-24",
                                  ...
                                  "participants": ["Alex Rivera"]}, written_by="live")

    def _nominated(self, text: str) -> list[str]:
        _same, _related, nominated, _cut = events.candidates(
            self.conn, people=["Cameron Ortiz"], entity="person:Cameron Ortiz",
            text=text)
        return [event.title for event in nominated]

    def test_a_person_the_message_names_nominates_their_row(self):
        self.assertIn("Ramen dinner",
                      self._nominated("Dinner is 8:30 now. Alex can't get there early."))

    def test_a_day_the_row_is_on_nominates_it(self):
        self.assertIn("Ramen dinner",
                      self._nominated("Still on for Thursday the 24th?"))

    def test_an_unrelated_message_nominates_nothing(self):
        self.assertEqual(self._nominated("did you ever find that charger"), [])


class TestBackfillProvesIdentityBeforeTouchingTheArchive(Base):
    """A UID means nothing without the UIDVALIDITY it was issued under.

    After a mailbox resync the same number addresses a different message, so fetching a
    stored UID and writing whatever comes back attaches one person's mail to another's
    archive row — with its evidence links, its citations and its place in a conversation
    all intact and all now wrong.
    """

    class Mailbox:
        """A folder whose UIDs can be renumbered, the way a resync renumbers them."""

        def __init__(self, by_uid, validity="1", unreadable=()):
            self.by_uid = by_uid
        ...
        def select(self, folder):
            if folder in self.unreadable:
                return False, ""
            return True, self.validity

        def message(self, uid):
            body = self.by_uid.get(int(uid))
            if body is None:
                return None
            return email.message_from_string(
                f"Message-ID: <{body[0]}>\nSubject: {body[1]}\n\n{body[2]}")
        ...
        def find(self, message_id):
            self.searched.append(message_id)
            for uid, body in self.by_uid.items():
                if body[0] == message_id:
                    return uid
            return None
    ...
    def _row(self, *, message_id, uid, validity, folder="INBOX",
        ...
        archive_id = archive.append(
            ...
            meta={"folder": folder, "uid": uid, "uidvalidity": validity,
                  "subject": "Your order", "body_error": "OSError: dropped"})
        self.conn.commit()
        return archive_id

    def _run(self, mailbox):
        real = proton.Bridge
        proton.Bridge = lambda _cfg: _Ctx(mailbox)
        try:
            return proton.backfill_bodies(self.conn, self.cfg, dry_run=False)
        finally:
            proton.Bridge = real

    def test_a_renumbered_uid_does_not_attach_the_wrong_body(self):
        row_id = self._row(message_id="mine@example.com", uid=7, validity="1")
        # A resync: UID 7 now holds somebody else's mail, and ours has moved to 12.
        mailbox = self.Mailbox({7: ("other@example.com", "Not yours", "someone else"),
                                12: ("mine@example.com", "Your order", "collect by Friday")},
                               validity="2")
        report = self._run(mailbox)
        text = self.conn.execute("SELECT text FROM archive WHERE id = ?",
                                 (row_id,)).fetchone()["text"]
        self.assertIn("collect by Friday", text)
        self.assertNotIn("someone else", text)
        self.assertEqual(report["relocated"], 1)

    def test_an_unfindable_message_leaves_the_row_exactly_as_it_was(self):
        row_id = self._row(message_id="gone@example.com", uid=7, validity="1")
        before = self.conn.execute("SELECT text, meta FROM archive WHERE id = ?",
                                   (row_id,)).fetchone()
        report = self._run(self.Mailbox(
            {7: ("other@example.com", "Not yours", "someone else")}, validity="2"))
        after = self.conn.execute("SELECT text, meta FROM archive WHERE id = ?",
                                  (row_id,)).fetchone()
        self.assertEqual(after["text"], before["text"])
        self.assertEqual(after["meta"], before["meta"])
        self.assertEqual(report["unidentified"], 1)
        self.assertEqual(report["updated"], 0)

    def test_a_uid_holding_a_different_message_is_rejected_even_when_validity_matches(self):
        """Servers do keep UIDVALIDITY across rebuilds, so the id is checked as well."""
        row_id = self._row(message_id="mine@example.com", uid=7, validity="1")
        report = self._run(self.Mailbox(
            {7: ("other@example.com", "Not yours", "someone else")}, validity="1"))
        text = self.conn.execute("SELECT text FROM archive WHERE id = ?",
                                 (row_id,)).fetchone()["text"]
        self.assertNotIn("someone else", text)
        self.assertEqual(report["unidentified"], 1)

    def test_a_folder_that_will_not_open_does_not_erase_another_folders_validity(self):
        """One unreadable folder used to take the selected one's identity with it.

        A failed select assigned its empty validity over the live one while leaving
        `current` pointing at the folder still selected — so every later row there took
        the already-selected path with no validity, skipped the identity check, and had
        its own `uidvalidity` overwritten with the blank.
        """
        self._row(message_id="first@example.com", uid=7, validity="1",
                  ts="2026-09-07T12:00:00")
        self._row(message_id="stuck@example.com", uid=3, validity="1",
                  folder="Archive", ts="2026-09-07T11:00:00")
        later = self._row(message_id="second@example.com", uid=8, validity="1",
                          ts="2026-09-07T10:00:00")
        report = self._run(self.Mailbox(
            {7: ("first@example.com", "Your order", "one"),
             8: ("second@example.com", "Your order", "two")},
            validity="1", unreadable={"Archive"}))
        meta = db.jload(self.conn.execute(
            "SELECT meta FROM archive WHERE id = ?", (later,)).fetchone()["meta"], {})
        self.assertEqual(meta["uidvalidity"], "1")
        self.assertEqual(report["updated"], 2)

    def test_a_row_whose_body_would_not_parse_is_failed_and_not_also_updated(self):
        """The counts have to sum to `attempted`, and the row learned nothing."""
        row_id = self._row(message_id="mine@example.com", uid=7, validity="1")
        before = self.conn.execute("SELECT text FROM archive WHERE id = ?",
                                   (row_id,)).fetchone()["text"]
        real = proton.extract_text
        proton.extract_text = lambda _m: (_ for _ in ()).throw(ValueError("bad MIME"))
        try:
            report = self._run(self.Mailbox(
                {7: ("mine@example.com", "Your order", "anything")}))
        finally:
            proton.extract_text = real
        self.assertEqual(report["failed"], 1)
        self.assertEqual(report["updated"], 0)
        after = self.conn.execute("SELECT text, meta FROM archive WHERE id = ?",
                                  (row_id,)).fetchone()
        self.assertEqual(after["text"], before)
        self.assertIn("bad MIME", db.jload(after["meta"], {})["body_error"])


class _Ctx:
    """Minimal context-manager wrapper so a fake mailbox can stand in for `Bridge`."""
...
class TestNormalisingMessageIdsDoesNotDuplicateArchivedMail(Base):
    """Existing rows are keyed with the angle brackets the header carries.

    Writing the bare form for the same message inserts a second row for it on the next
    collection — the same mail twice, under two spellings, each accumulating its own
    evidence links.
    """
    ...
    def _deliver(self, message_id):
        ...
        message = email.message_from_string(
            ...
            f"From: Shop <shop@example.com>\nTo: Casey <casey@example.com>\n"
            f"Subject: Your order\n\nbody")
        proton._handle_message(self.conn, self.cfg, report, self.Mailbox(), 1,
                               message, "INBOX")
    ...
    def test_a_reply_joins_an_ancestor_archived_under_the_old_spelling(self):
        ...
        self.conn.commit()
        self.assertEqual(
            proton.conversation_key(self.conn, own="reply@example.com", parent="",
                                    chain=["root@example.com"],
                                    fallback="shop@example.com"),
            "root@example.com")
...
class TestApplyDatesEachFieldFromTheLinesTheReaderCited(Base):
    """Per-field support has to come from the reader, not from matching text.

    A field's timestamp decides whether it may overrule something the user corrected, and
    every way code has of guessing it is wrong in a different direction: a quoted old
    arrangement looks like a statement of it, a denial looks like an assertion, and an
    update phrased without the literal looks like no evidence at all.
    """

    def _bundle(self, lines) -> tuple[bundle_stage.Bundle, dict]:
        ids = {}
        ...
        return (bundle_stage.Bundle(entity="person:Alex Rivera", title="Alex Rivera",
                                    items=rows, spool_ids=[]), ids)

    def _apply(self, bundle, row):
        apply_stage.apply_diffs(self.conn, self.cfg, [(bundle, {
            "events": [row], "todos": [], "wiki": [], "questions": []})],
            written_by="dream:nightly")
        return events.get(self.conn, events.make_key("Brunch", "2026-09-20"))

    def setUp(self):
        super().setUp()
        db.set_today("2026-09-08T09:00:00")
        self.addCleanup(db.set_today, None)
        live.add_event(self.conn, self.cfg, title="Brunch", when="2026-09-20",
                       location="Blue Fern", time="11:00")

    def test_a_cited_field_lands_and_an_older_cited_field_does_not(self):
        bundle, ids = self._bundle([
            ("place", "2026-09-07T18:30:00", "brunch is at Rosewood, 11am"),
            ("time", "2026-09-08T20:00:00", "make it 11:30 instead"),
        ])
        db.set_today("2026-09-08T23:30:00")
        row = self._apply(bundle, {
            "title": "Brunch", "date": "2026-09-20", "time": "11:30",
            "location": "Rosewood", "subject": "me",
            "field_cite_ids": {"time": [ids["time"]], "location": [ids["place"]]}})
        self.assertEqual(row.time, "11:30")
        self.assertEqual(row.location, "Blue Fern")

    def test_a_line_quoting_the_old_arrangement_is_not_evidence_for_it(self):
        """The quoted line is newer than the correction, and says nothing new."""
        bundle, ids = self._bundle([
            ("real", "2026-09-08T19:00:00", "we are still on for the 20th"),
            ("quote", "2026-09-08T20:00:00",
             "cam keeps saying \u201cbrunch is at Rosewood\u201d, he missed the change"),
        ])
        db.set_today("2026-09-08T23:30:00")
        row = self._apply(bundle, {
            "title": "Brunch", "date": "2026-09-20", "subject": "me",
            "location": "Rosewood",
            # Nothing here states the place; the reader cites nothing for it.
            "field_cite_ids": {"date": [ids["real"]]}})
        self.assertEqual(row.location, "Blue Fern")

    def test_a_denial_of_the_old_place_does_not_reinstate_it(self):
        bundle, ids = self._bundle([
            ("deny", "2026-09-08T20:00:00", "it is not at Rosewood any more"),
        ])
        db.set_today("2026-09-08T23:30:00")
        row = self._apply(bundle, {
            "title": "Brunch", "date": "2026-09-20", "subject": "me",
            "location": "Rosewood", "field_cite_ids": {}})
        self.assertEqual(row.location, "Blue Fern")

    def test_an_update_that_never_repeats_the_value_is_still_supported(self):
        """"push it back an hour" carries no literal to match, and is the evidence.

        The older line is what makes this discriminating: it is the only one naming the
        occasion, so anything inferring support from the text dates the new time from
        the *seventh* — before the correction — and refuses it.
        """
        bundle, ids = self._bundle([
            ("old", "2026-09-07T12:00:00", "brunch on the 20th, 11am then"),
            ("shift", "2026-09-08T20:00:00", "can we push it back an hour?"),
        ])
        db.set_today("2026-09-08T23:30:00")
        row = self._apply(bundle, {
            "title": "Brunch", "date": "2026-09-20", "time": "12:00", "subject": "me",
            "field_cite_ids": {"time": [ids["shift"]]}})
        self.assertEqual(row.time, "12:00")

    def test_a_citation_naming_no_line_of_this_bundle_grants_nothing(self):
        bundle, _ids = self._bundle([
            ("only", "2026-09-08T20:00:00", "still on for the 20th"),
        ])
        db.set_today("2026-09-08T23:30:00")
        row = self._apply(bundle, {
            "title": "Brunch", "date": "2026-09-20", "subject": "me",
            "location": "Rosewood", "field_cite_ids": {"location": [999999]}})
        self.assertEqual(row.location, "Blue Fern")
