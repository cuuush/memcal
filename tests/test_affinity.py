...
class TestTheUserIsNotOneVoice(unittest.TestCase):
    ...
    def test_an_unattributed_line_is_credited_to_its_conversation(self):
        a = fragments(Bundle("person:marco", [
            line("beer garden next Sunday in Midtown", "2026-07-25T12:08:37-04:00")]))
...
class TestRelatednessNeedsMoreThanAWord(unittest.TestCase):
    def test_a_shared_word_on_an_agreeing_day_links(self):
        ...
        b = frag("b", "2026-08-02", {"beer", "harborview"})
...
class TestGroupingStaysBounded(unittest.TestCase):
    ...
    def test_a_seed_pair_over_budget_is_not_a_group(self):
        """The one place the budget was not tested. The accretion loop checks every
        later member and `propose.pack` appends what comes back verbatim, so a pair
        could leave here at several times `pack_tokens` — and a 26,467-token bundle is
        on record. Both fall through as leftovers, where `pack` decides and where one
        big bundle is allowed to ride alone."""
        many = [Bundle(f"e{i}", [line("beer garden sunday", "2026-07-27T10:00:00-04:00")])
                for i in range(4)]
        groups, leftovers = group(many, max_bundles=6, max_tokens=15, cost=lambda b: 10)
        self.assertEqual(groups, [])
        self.assertEqual(len(leftovers), 4)

    def test_a_pair_that_fits_is_still_grouped(self):
        """The counterweight: the check must reject the pair that does not fit, not
        stop pairing."""
        many = [Bundle(f"e{i}", [line("beer garden sunday", "2026-07-27T10:00:00-04:00")])
                for i in range(4)]
        groups, _left = group(many, max_bundles=6, max_tokens=20, cost=lambda b: 10)
