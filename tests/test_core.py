...
class TestMatching(Base):
    ...
    def test_the_same_title_on_the_same_day_is_the_same_occasion(self):
        events.upsert(self.conn, {"title": "dinner with Alex", "date": self.d(2)})
        _, verb = events.upsert(self.conn, {"title": "Dinner With Alex", "date": self.d(2),
                                            "status": "confirmed"})
        self.assertEqual(verb, "updated")

    def test_the_same_title_on_another_day_is_another_occasion(self):
        """Two dinners with Alex on consecutive days are two dinners.

        A title is what an occasion is called, and everything a provider books is called
        the same thing — so a name shared across days is a reason to look at the other
        row, not a decision that they are one. A mention that means to move an existing
        row says so with its key.
        """
        first, _ = events.upsert(self.conn, {"title": "dinner with Alex",
                                             "date": self.d(2)})
        _, verb = events.upsert(self.conn, {"title": "Dinner With Alex", "date": self.d(3),
                                            "status": "confirmed"})
        self.assertEqual(verb, "inserted")
        self.assertEqual(len(events.window(self.conn, 3, 10)), 2)
        # …and the same wording *with* the row identified is the reschedule it claims.
        moved, verb = events.upsert(self.conn, {"key": first.key,
                                                "title": "Dinner With Alex",
                                                "date": self.d(4)})
        self.assertEqual(verb, "updated")
        self.assertEqual(moved.date, self.d(4))

    def test_a_restatement_of_a_day_the_row_has_held_still_finds_it(self):
        """Otherwise every straggler repeating the old date becomes a duplicate."""
        first, _ = events.upsert(self.conn, {"title": "Brunch", "date": self.d(2)})
        events.upsert(self.conn, {"key": first.key, "title": "Brunch",
                                  "date": self.d(3)})
        found, verb = events.upsert(self.conn, {"title": "Brunch", "date": self.d(2),
                                                "location": "Rosewood"})
        self.assertEqual(verb, "updated")
        self.assertEqual(found.key, first.key)
    ...
    def test_merging_duplicates_keeps_every_link_to_the_source(self):
        keep, _ = events.upsert(self.conn, {"title": "Work on song", "date": self.d(1)})
        ...
        trace.stamp(self.conn, kind="event", ref=drop.key, verb="inserted",
                    entity="person:Quentin", archive_ids=[archive_id])
        todo, _ = todos.open_todo(self.conn, "Bring the demo", event_id=drop.id)
        merged = events.merge(self.conn, keep.key, drop.key)

        self.assertEqual(merged.id, keep.id)
        self.assertEqual(todos.get(self.conn, todo.key).event_id, keep.id)
        self.assertTrue(self.conn.execute(
            "SELECT 1 FROM evidence WHERE kind='event' AND ref=? AND archive_id=?",
            (keep.key, archive_id),
        ).fetchone())
        self.assertFalse(self.conn.execute(
            "SELECT 1 FROM provenance WHERE kind='event' AND ref=?", (drop.key,)
        ).fetchone())
...
class TestGate(Base):
    ...
    def test_email_sender_table_is_a_lookup_after_one_decision(self):
        ...
        # *when* this sender is read, not whether they exist.
    ...
    def test_kindred_can_be_flipped_off_by_hand(self):
        # A person saying no is the one thing that still keeps mail out entirely.
        identity.set_sender(self.conn, "hello@kindred.com", "ignore", "don't care",
                            source="you")
        verdict = gate.gate_email(self.conn, address="hello@kindred.com",
                                  subject="member event")
        self.assertFalse(verdict)
        self.assertTrue(verdict.reason.startswith("blocked:"))

    def test_an_old_automatic_ignore_is_a_ranking_and_not_a_permanent_block(self):
        """The half of the same table that must not behave like a decision anybody made.

        `auto` rows are the gate's own bookkeeping. Left as exclusions they became
        permanent blocks nobody chose and nobody could see, on senders whose only crime
        was carrying a List-Id in 2024.
        """
        identity.set_sender(self.conn, "hello@shop.example", "ignore", "bulk-headers")
        verdict = gate.gate_email(self.conn, address="hello@shop.example",
                                  subject="anything at all")
        self.assertTrue(verdict.low)
        self.assertEqual(verdict.reason, "sender-table:ignore")
...
class TestPacking(Base):
    ...
    def test_warming_the_cache_first_does_not_reorder_results(self):
        ...
        good, errors, _notes = propose_stage.propose_all(
    ...
    def test_a_huge_bundle_does_not_drag_others_over_budget(self):
        ...
        sizes = [textclean.estimate_tokens(propose_stage.build_suffix(self.cfg, g)) for g in groups]
...
class TestICalendar(Base):
    ...
    def test_subscribed_holiday_is_not_a_memcal_event(self):
        report = self.snapshot([
            self.item("rosh-hashanah", "Rosh Hashanah", 4,
                      calendar="US Holidays", writable=False),
        ])
        self.assertEqual(events.window(self.conn, 0, 10), [])
        self.assertFalse(self.conn.execute("SELECT 1 FROM calendar_items").fetchone())
        self.assertTrue(any("holiday calendars" in note for note in report.notes))

    def test_a_feed_renamed_to_holidays_removes_its_old_row(self):
        first = self.item("rosh-hashanah", "Rosh Hashanah", 4,
                          calendar="Reference", writable=False)
        self.snapshot([first])
        holiday = dict(first, calendar_name="US Holidays")
        self.snapshot([holiday])
        self.assertFalse(self.conn.execute("SELECT 1 FROM events").fetchone())
        self.assertFalse(self.conn.execute("SELECT 1 FROM calendar_items").fetchone())

    def test_partiful_hosts_are_separate_from_guests_and_visible_when_useful(self):
    ...
    def test_a_host_already_named_by_the_title_is_not_repeated(self):
        invitation = self.item("birthday", "Katie’s 30th | Partiful", 5,
                               calendar="Partiful", writable=False)
        invitation["hosts"] = ["Katie O’Rourke"]
        self.snapshot([invitation])
        self.assertNotIn("hosted by", events.window(self.conn, 0, 10)[0].one_line())

    def test_public_partiful_hosts_are_cached_after_the_first_lookup(self):
        invitation = self.item("movie", "Movie", 5, calendar="Partiful", writable=False)
        invitation["url"] = "https://partiful.com/e/movie"
        pages = {
        ...
        self.assertEqual(providers.partiful.enrich_hosts([invitation], fetch=fetch), 1)
        self.snapshot([invitation])
        fresh = dict(invitation)
        fresh.pop("hosts")
        self.assertEqual(
            providers.partiful.enrich_hosts(
                [fresh], conn=self.conn,
                fetch=lambda _url: self.fail("cached host names should prevent a fetch"),
            ),
            0,
        )
...
class TestTextClean(Base):
    ...
    def test_a_wall_of_blanks_still_costs_something(self):
        # Whitespace is nearly free per the fit, which holds for cleaned text and not
        # for a run of blanks a tokenizer charges for in groups.
        self.assertGreater(textclean.estimate_tokens(" " * 500), 20)


class TestTokenEstimateNeverFallsBelowTheRealCount(unittest.TestCase):
    """Issue #20: the docstring promised over-counting and delivered 0.854 of the
    provider's own `usage.prompt_tokens` on all 162 saved calls. `tests/token_calibration.json`
    is those calls reduced to character-class counts — no message text — so the promise
    stays checkable in a public repository. Refresh it with `tools/token_calibration.py`."""

    FIXTURE = Path(__file__).resolve().parent / "token_calibration.json"

    @classmethod
    def setUpClass(cls):
        cls.rows = json.loads(cls.FIXTURE.read_text(encoding="utf-8"))

    def stand_in(self, row):
        """Text with the row's character-class counts. The estimate reads only counts."""
        return ("a" * row["letters"] + " " * row["spaces"] + "1" * row["digits"]
                + "." * row["marks"] + "—" * row["wide"])

    def test_the_fixture_carries_counts_and_no_message_text(self):
        self.assertGreaterEqual(len(self.rows), 100)
        for row in self.rows:
            self.assertEqual(set(row), {"letters", "spaces", "digits", "marks", "wide", "tokens"})
            self.assertTrue(all(isinstance(value, int) and value >= 0 for value in row.values()))

    def test_no_saved_call_is_under_counted(self):
        light = [(textclean.estimate_tokens(self.stand_in(row)), row["tokens"])
                 for row in self.rows]
        short = [f"{est} < {real}" for est, real in light if est < real]
        self.assertEqual(short, [], f"{len(short)} of {len(light)} calls under-counted")

    def test_the_margin_over_the_corpus_stays_small(self):
        estimated = sum(textclean.estimate_tokens(self.stand_in(row)) for row in self.rows)
        real = sum(row["tokens"] for row in self.rows)
        # Over-counting is the safe direction, but a wild margin would starve packing.
        self.assertGreater(estimated / real, 1.0)
        self.assertLess(estimated / real, 1.30)
...
class TestSessionRegressions(Base):
    ...
    def test_resolve_closes_a_todo_not_just_a_question(self):
        todos.open_todo(self.conn, "Pick up Target order #100200300400 at Herald Square")
    ...
    def test_real_questions_are_untouched(self):
        good = "You were at Lakeview on Tuesday — did you give Rowan that EZ-Pass back?"
    ...
    def test_the_cap_holds_even_when_the_marker_is_added(self):
        ...
        self.assertLessEqual(textclean.estimate_tokens(brief.render(self.conn, self.cfg)), 90)
...
class TestUserIdentity(Base):
    ...
    def test_a_row_about_him_is_subject_me(self):
        # Live failure: key was `casey:lakeview-cabin-trip-with-harper@2026-07-15`.
        diff = {"events": [{"title": "Lakeview cabin trip with Harper",
...
class TestQuestionHygiene(Base):
    ...
    def test_a_wake_does_not_repeat_dreams_question_about_the_same_todo(self):
        todo, _ = todos.open_todo(
            self.conn, "Give Rowan back their EZ-Pass",
            wake_condition="Rowan is back from Italy",
        )
        first = todos.ask(
            self.conn, "Rowan is back from Italy — did you return their EZ-Pass?",
            about_todo=todo.id, written_by="dream:nightly",
        )
        wake = todos.ask(
            self.conn,
            "Give Rowan back their EZ-Pass — Rowan is back from Italy now looks true. "
            "Still open?",
            key=f"q:wake:{todo.key}", about_todo=todo.id, written_by="dream",
        )
        self.assertEqual(wake, first)
        self.assertEqual(len(todos.open_questions(self.conn)), 1)
...
class TestAQuestionAskedAboutADayHeSpentElsewhere(Base):
    ...
    def test_the_refusal_leaves_a_trace(self):
        ...
        self.assertEqual(len(rows), 1)
        self.assertIn("Solstice festival", rows[0]["entity"])
    ...
    def test_a_nested_row_is_the_same_occasion_not_a_clash(self):
        ...
        self.conn.commit()
        self.assertNotEqual(
            todos.ask(self.conn, "Did Breakfast at Solstice happen on Sunday?",
...
class TestASeriesDidNotCarryItsQualitiesToAMovedInstance(Base):
    ...
    def test_a_judgement_is_never_lent(self):
        ...
        `SERIES_QUALITIES` covers where it is and how you join it, and stops
    ...
    def test_the_newest_instance_is_the_one_that_lends(self):
        """A series that changed venue carries the newest one."""
    ...
    def test_a_weak_title_match_does_not_lend_anything(self):
        ...
        It absorbs "Solstice" into "Breakfast at Solstice", which is the collision
...
class TestAnOccurrenceWhoseRuleIsNotPublishedStaysOff(Base):
    """`publish_pending` skipped an occurrence only when a *published* rule covered it,
    so a row carrying a series slug whose rule is not out there published individually —
    and nothing retracts those. Twenty rows in the store carry a slug with no rule
    behind it at all, and EventKit access is a separate grant from the Apple Events
    access the scan uses, so one weekly series whose `publish_series` was refused is 52
    standalone events a year that no retraction path knows about."""

    def _runner(self, command, **kwargs):
        self.sent.append(command)
        raise AssertionError("published an occurrence whose rule is not on the calendar")

    def test_an_orphan_slug_does_not_publish_on_its_own(self):
        self.sent = []
        self.cfg.publish_calendar = "memcal"
        events.upsert(self.conn, {
            "title": "Tutoring", "date": self.d(2), "time": "13:00",
            "kind": "commitment", "status": "confirmed", "series": "tutoring"},
            written_by="live")
        log = ical.publish_pending(self.conn, self.cfg, runner=self._runner)
        self.assertEqual(self.sent, [])
        self.assertTrue(any("not published" in line for line in log), log)

    def test_a_row_with_no_series_still_publishes(self):
        """The counterweight. "Publish nothing" is the same bug pointed the other way,
        and an ordinary confirmed row is what the feature is for."""
        self.cfg.publish_calendar = "memcal"
        events.upsert(self.conn, {
            "title": "Poker at Jordan's", "date": self.d(2), "kind": "commitment",
            "status": "confirmed"}, written_by="live")
        reached = []

        def runner(command, **kwargs):
            reached.append(command)
            raise RuntimeError("stop here; reaching osascript is the assertion")

        with contextlib.suppress(Exception):
            ical.publish_pending(self.conn, self.cfg, runner=runner)
        self.assertTrue(reached, "an ordinary confirmed row must still be published")

    def test_the_exception_that_contradicts_a_rule_still_publishes(self):
        """`instead_of` is the row that disagrees with the schedule, so it is exactly
        the one that needs a copy of its own — the skip must not swallow it."""
        self.cfg.publish_calendar = "memcal"
        events.upsert(self.conn, {
            "title": "Tutoring", "date": self.d(3), "time": "12:00",
            "kind": "commitment", "status": "confirmed", "series": "tutoring",
            "instead_of": self.d(2)}, written_by="live")
        reached = []
...
class TestAnInferenceCannotOverwriteAnObservation(Base):
    ...
    def _ical_row(self):
        event, _ = self.conn and events.upsert(
            self.conn, {"title": "solstice", "date": self.d(7),
    ...
    def test_a_chat_proposal_cannot_move_a_subscribed_date(self):
        ...
        after, verb = events.upsert(
            self.conn, {"title": "Solstice", "date": self.d(1),
    ...
    def test_origin_records_where_a_row_came_from_not_who_touched_it_last(self):
        ...
        ical = self._ical_row()
        after, _ = events.upsert(
            self.conn, {"title": "Solstice", "date": ical.date,
...
class TestWeakMatchBoundaries(Base):
    ...
    Quinn and Avery: one shared friend, and the word "quinn" in both titles doing
    duty as though it were independent corroboration.
    """

    def test_a_shared_name_alone_does_not_merge_two_occasions(self):
        events.upsert(self.conn, {
            "title": "Lunch then packs at Demo Court with Quinn and Avery",
    ...
    def test_a_weak_match_does_not_reach_across_today(self):
        ...
        _, verb = events.upsert(self.conn, {"title": "drinks", "date": self.d(4),
                                            "participants": ["Quinn Brooks"]})
        # Drinks last week and drinks next week are two evenings. Sharing a name across
        # days stopped being a claim that they are one occasion — the row that means to
        # move an existing plan identifies it.
        self.assertEqual(verb, "inserted")
        self.assertEqual(len(events.window(self.conn, 10, 10)), 2)
    ...
    def test_a_weak_match_pools_detail_without_moving_the_date(self):
        ...
        settled, _ = events.upsert(self.conn, {
            "title": "Beer garden at Harborview Hall", "date": self.d(6),
            "time": "after 6", "participants": ["Quinn Brooks", "Jamie"]})
        merged, verb = events.upsert(self.conn, {
            "title": "Beer garden with Avery", "date": self.d(5),
...
class TestWikiPagesEarnTheirPlace(Base):
    ...
    def test_a_renamed_series_retires_its_generated_predecessor(self):
        event, _ = events.upsert(
            self.conn,
            {"title": "Session with Taylor", "date": self.d(1),
             "series": "session-with-taylor", "location": "Virtual"},
            match=False,
        )
        wiki.ensure_series(self.conn, self.cfg.wiki_dir, "session-with-taylor")
        self.conn.execute("UPDATE events SET series='piano-lesson' WHERE id=?", (event.id,))
        self.conn.commit()

        self.assertEqual(
            wiki.retire_obsolete_series(self.conn, self.cfg.wiki_dir),
            ["session-with-taylor"],
        )
        self.assertIsNone(wiki.read(self.cfg.wiki_dir, "session-with-taylor"))
        self.assertTrue(
            (self.cfg.wiki_dir / ".retired" / "projects" /
             "session-with-taylor.md").exists()
        )

    def test_an_orphaned_series_page_with_a_human_fact_is_kept(self):
        wiki.set_slot(
            self.cfg.wiki_dir, "old-project", "where", "the studio",
            source="you", section="projects", conn=self.conn,
        )
        self.assertEqual(wiki.retire_obsolete_series(self.conn, self.cfg.wiki_dir), [])
        self.assertIsNotNone(wiki.read(self.cfg.wiki_dir, "old-project"))

    def test_a_generated_page_with_current_series_rows_is_kept(self):
        events.upsert(
            self.conn,
            {"title": "Poker night", "date": self.d(1), "series": "poker-night"},
            match=False,
        )
        wiki.ensure_series(self.conn, self.cfg.wiki_dir, "poker-night")
        self.assertEqual(wiki.retire_obsolete_series(self.conn, self.cfg.wiki_dir), [])
        self.assertIsNotNone(wiki.read(self.cfg.wiki_dir, "poker-night"))
...
class TestCrossPlatformIdentity(Base):
    ...
    def test_three_streams_land_in_one_bundle(self):
        self._deliver("imessage", "+17575550115", "beer garden sunday?")
        self._deliver("whatsapp", whatsapp.phone_of("17575550115@s.whatsapp.net"),
                      "still on for sunday")
    ...
    def test_a_whatsapp_jid_and_an_imessage_number_are_the_same_person(self):
        self._deliver("imessage", "+17575550115", "you free sunday?")
        self._deliver("whatsapp", whatsapp.phone_of("17575550115@s.whatsapp.net"),
                      "yeah sunday works")
...
class TestThreadIdentity(Base):
    ...
    def _msg(self, ext, text, *, person=None, from_me=False, thread=None, is_group=False):
        ...
        sources.deliver(self.conn, report, stream="imessage", external_id=ext,
                        ...
                        counterpart=None if from_me else "+16095550112")
    ...
    def test_his_side_joins_the_person_when_the_thread_is_a_dm(self):
        identity.link(self.conn, "+16095550112", "Avery Morgan")
...
class TestEntityIsAPerson(Base):
    ...
    def test_a_resolved_handle_still_keys_on_the_person(self):
        identity.link(self.conn, "+16095550112", "Avery Morgan")
        entity = self._entity_for(thread="+16095550112", handle="+16095550112",
                                  counterpart="+16095550112")
...
class TestResolvingWhatIsAlreadyDone(Base):
    ...
    def test_an_already_closed_todo_reports_settled_not_missing(self):
        todo, _ = todos.open_todo(self.conn, "Mail the signed paperwork back")
        todos.close(self.conn, todo.key)
        ok, kind = todos.resolve(self.conn, "Mail the signed paperwork back",
...
class TestQuestionsExpire(Base):
    ...
    def test_a_stale_question_is_dropped(self):
        todos.ask(self.conn, "Which day next weekend is Avery coming over?")
...
class TestAgeIsNotADeadline(Base):
    ...
    def test_a_real_due_date_still_reads_as_one(self):
        todos.open_todo(self.conn, "Sign the paperwork", due=self.d(6))
        line = todos.open_items(self.conn)[0].one_line()
        self.assertIn("opened today", line)
...
class TestAChatCanBeMuted(Base):
    ...
    def test_a_chat_he_posts_in_is_never_raised(self):
        self.busy_chat("BGC Gamin", mine=3)
        threads.refresh(self.conn)
...
class TestAnAmendmentFindsWhatItAmends(Base):
    ...
    def test_a_silent_generic_message_pass_gets_one_event_graph_retry(self):
        ...
        self.cfg.pack_bundles = 1
        proposed, errors, notes = propose_stage.propose_all(
        ...
        # A second look is the machinery recovering, not the pass failing: it must not
        # reach `runs.error`, which is what `memcal doctor` reads.
        self.assertEqual(errors, [])
        self.assertTrue(any("event graph" in note for note in notes))
    ...
    def test_a_row_written_out_of_this_very_conversation_is_marked(self):
        self.settled_it("thread:groupme:BGC Gamin", "beer garden saturday?")
        bundle = self.amendment("thread:groupme:BGC Gamin", "cant do saturday")
        bundle.entity = "thread:groupme:BGC Gamin"
        block = propose_stage.build_open_rows(self.conn, bundle)
        self.assertIn("←from here", block)

    def test_somebody_elses_plan_is_not_offered(self):
        events.upsert(self.conn,
                      {"date": self.saturday, "kind": "commitment", "subject": "Robin",
                       "title": "Robin's dentist", "status": "confirmed",
                       "participants": ["Robin"]}, written_by="dream:nightly")
    ...
    def test_an_event_name_alone_does_not_create_a_global_edge(self):
        old, _ = events.upsert(
            ...
            {"date": self.d(-15), "kind": "observed", "subject": "me",
             "title": "Solstice meetup", "status": "happened",
             "participants": []},
            written_by="live")
        upcoming, _ = events.upsert(
            self.conn,
            {"date": self.d(8), "kind": "commitment", "subject": "me",
             "title": "Solstice Music & Arts Festival", "status": "confirmed",
             "participants": []},
            written_by="live", match=False)
        ...
        bundle = self.amendment(
            ...
            "I got the Solstice tickets but no word on the car ticket",
            person=None)
        block = propose_stage.build_open_rows(self.conn, bundle)
...
class TestTheLaterBlockIsAboutThingsHeIsDoing(Base):
    ...
    def test_an_opaque_partiful_title_names_its_hosts(self):
        events.upsert(
            self.conn,
    ...
    def test_a_birthday_title_does_not_repeat_the_host(self):
        events.upsert(
            self.conn,
            {"title": "Katie's 30th | Partiful", "date": "2026-08-22",
             "kind": "commitment", "status": "confirmed", "hosts": ["Katie"]},
            written_by="ical",
        )
        self.assertNotIn("hosted by", self._later())
...
class TestAQuestionAboutSomethingNobodyMentioned(Base):
    ...
    def test_one_name_present_is_enough(self):
        ...
        bundle = self._bundle(["harborview hall sunday?"])
        self.assertFalse(apply_stage._talks_about_nothing_here(
            bundle, "Is the Harborview Hall outing still on, and is Aaron confirmed?"))
...
class TestNoTestEverWritesToTheRealCalendar(Base):
    ...
    def test_a_checkout_env_cannot_switch_publishing_on_for_another_home(self):
        """Outward writing is opted into from *the store's own*
        `.env`. `config.load` merges three files — the checkout's, the home's and the
        cwd's — so `MEMCAL_PUBLISH_CALENDAR` in the checkout, which exists here and is
        gitignored, switched it on for every home the process ever opened, scratch homes
        included."""
        from memcal import config as config_mod
        with tempfile.TemporaryDirectory() as tmp:
            ...
            (checkout / ".env").write_text(
                "MEMCAL_PUBLISH_CALENDAR=Real Calendar\nMEMCAL_PACK_BUNDLES=3\n",
                encoding="utf-8")
            with mock.patch.object(config_mod, "PROJECT_ROOT", checkout):
                cfg = config_mod.load(home)
        self.assertEqual(cfg.publish_calendar, "")
        # ...and the ordinary knobs still merge, which is the convenience being kept.
        self.assertEqual(cfg.pack_bundles, 3)

    def test_the_stores_own_env_still_switches_it_on(self):
        """The counterweight. "Never read a file" is the same bug pointed the other way:
        the documented way to turn publishing on has to keep working."""
        from memcal import config as config_mod
        with tempfile.TemporaryDirectory() as tmp:
            home = Path(tmp)
            (home / ".env").write_text("MEMCAL_PUBLISH_CALENDAR=memcal\n",
                                       encoding="utf-8")
            self.assertEqual(config_mod.load(home).publish_calendar, "memcal")
...
class TestEveryTestInThisDirectoryActuallyRuns(Base):
    ...
    def _collected_by_running_directly(self, path):
        """What `python3 <file>` would collect, without running one test.

        `unittest.main` is swapped for a recorder, so the file's own entry point fires
        exactly where it sits and reports what it can see from there. The version this
        replaces compared two counts that both came from *importing* the module, where
        `__main__` never fires — so neither side could be short and it always passed.
        """
        captured = {}

        def recorder(*_args, **_kwargs):
            captured["count"] = unittest.defaultTestLoader.loadTestsFromModule(
                sys.modules["__main__"]).countTestCases()

        real = unittest.main
        unittest.main = recorder
        try:
    ...
    def test_running_a_file_directly_collects_what_discovery_does(self):
        here = Path(__file__).resolve().parent
        self.assertTrue(self._files())
        for path in self._files():
            found = unittest.defaultTestLoader.discover(str(here), pattern=path.name)
            self.assertEqual(self._collected_by_running_directly(path),
                             found.countTestCases(),
                             f"{path.name}: a direct run and discovery disagree")

    def test_a_class_after_the_entry_point_is_caught(self):
        """The decoy, and the whole reason the two checks above exist. One test is
        invisible to a direct run and both counts have to notice."""
        with tempfile.TemporaryDirectory() as tmp:
            path = Path(tmp) / "test_middle_entry_point.py"
            path.write_text(
                "import unittest\n"
                "class TestBeforeTheEntryPoint(unittest.TestCase):\n"
                "    def test_a(self):\n"
                "        pass\n"
                "if __name__ == '__main__':\n"
                "    unittest.main()\n"
                "class TestAfterTheEntryPoint(unittest.TestCase):\n"
                "    def test_b(self):\n"
                "        pass\n", encoding="utf-8")
            self.assertEqual(self._collected_by_running_directly(path), 1)
            self.assertEqual(unittest.defaultTestLoader.discover(
                tmp, pattern=path.name).countTestCases(), 2)
...
class TestAFixtureThatNamesTwoRowsGetsTwoRows(Base):
    """`upsert` defaults to `match=True`, so two calls can come back as one row.

    Writing #58's control, "League or CS2 gaming" and "League gaming" returned one id.
    That one failed loudly. Its siblings assert `assertIsNone(part_of)`, which is green
    on a store holding one row and looks exactly like a test that holds. Naming two
    results is a claim that there are two rows; this asks for the proof — `match=False`
    on the later ones, or an `assertNotEqual` on the ids.
    """

    def _upserts(self, fn):
        return [c for c in ast.walk(fn) if isinstance(c, ast.Call)
                and ast.unparse(c.func).endswith("events.upsert")]

    def _bound(self, fn):
        """Each `events.upsert` call in `fn` -> the names its row and verb were given."""
        out = {}
        for node in ast.walk(fn):
            if not (isinstance(node, ast.Assign) and isinstance(node.value, ast.Call)):
                continue
            if not ast.unparse(node.value.func).endswith("events.upsert"):
                continue
            target, row, verb = node.targets[0], None, None
            if isinstance(target, ast.Tuple) and len(target.elts) == 2:
                row, verb = (e.id if isinstance(e, ast.Name) else None
                             for e in target.elts)
            elif isinstance(target, ast.Name):
                row = target.id
            out[id(node.value)] = (row, verb)
        return out

    def _says(self, call, name):
        """`match=False` as an argument, or `"key": row.key` inside the fields dict."""
    ...
    def _frames(self, fn, fixture, methods):
        """Where this test's rows are built: itself, setUp, and helpers it calls.

        `TestAWrongNestingWasPermanent` builds the pair in `_bad_nesting()`, which is
        the fixture the issue was written about — a check that only reads test bodies
        would miss the one case that started it.
        """
        seen, out = set(), []
        for f in (*fixture, fn):
            for call in [c for c in ast.walk(f) if isinstance(c, ast.Call)]:
                helper = methods.get(ast.unparse(call.func).rsplit(".", 1)[-1])
                if (helper is not None and helper is not f
                        and helper.name not in seen and self._upserts(helper)):
                    seen.add(helper.name)
                    out.append(helper)
        return (*fixture, *out, fn)

    def _offenders(self, tree, label):
        found = []
        for cls in [c for c in ast.walk(tree) if isinstance(c, ast.ClassDef)]:
            ...
            for fn in [m for m in cls.body if isinstance(m, ast.FunctionDef)
                       and m.name.startswith("test")]:
                frames = self._frames(fn, fixture, methods)
                calls = [c for f in frames for c in self._upserts(f)]
                if len(calls) < 2:
                    continue
                bound = {}
                for f in frames:
                    bound.update(self._bound(f))
                named = [c for c in calls
                         if (bound.get(id(c), (None, None))[0] or "_")[0] != "_"]
                rows = [bound[id(c)][0] for c in named]
                if len(set(rows)) < 2:
                    continue                        # never claimed two rows
                if any(bound[id(c)][1] and bound[id(c)][1][0] != "_" for c in named):
                    continue                        # reads the verb, so it is watching
                if any(self._says(c, "key") for c in calls):
                    continue                        # names the row it means to land on
                if "assertNotEqual" in "\n".join(ast.unparse(f) for f in frames):
                    continue                        # proves it by hand
                if all(self._says(c, "match") == "False" for c in named[1:]):
                    continue                        # every later row is a new one
                found.append(f"{label}:{fn.lineno} {cls.name}.{fn.name} "
                             f"{sorted(set(rows))}")
        return found

    def test_every_fixture_that_names_two_rows_proves_they_are_two(self):
        offenders = [line for path in _test_sources()
                     for line in self._offenders(
                         ast.parse(path.read_text(encoding="utf-8")), path.name)]
        self.assertEqual(offenders, [],
                         "these name two rows and take on trust that upsert made two; "
                         "pass match=False on the later ones, or assertNotEqual the ids")

    def test_the_check_has_something_to_check(self):
        """`events.upsert` renamed, or the fixtures moved, and this reports nothing."""
        methods = 0
        for path in _test_sources():
            tree = ast.parse(path.read_text(encoding="utf-8"))
            for cls in [c for c in ast.walk(tree) if isinstance(c, ast.ClassDef)]:
                fns = [m for m in cls.body if isinstance(m, ast.FunctionDef)]
                methods += sum(1 for m in fns if m.name.startswith("test")
                               and len(self._upserts(m)) > 1)
        self.assertGreater(methods, 40)

    def test_the_fixture_that_started_this_is_flagged(self):
        """#58's control, reduced. The row named `window` is never asserted about and
        the assertion below is green on a store with one row in it."""
        tree = ast.parse(
            "class TestX:\n"
            "    def test_y(self):\n"
            "        window, _ = events.upsert(conn, {'title': 'League or CS2'})\n"
            "        session, _ = events.upsert(conn, {'title': 'League gaming'})\n"
            "        self.assertIsNone(get_by_id(session.id).part_of)\n")
        self.assertEqual(len(self._offenders(tree, "x.py")), 1)
        # ...and the same fixture saying what it wants is not flagged.
        said = ast.parse(ast.unparse(tree).replace(
            "{'title': 'League gaming'})", "{'title': 'League gaming'}, match=False)"))
        self.assertEqual(self._offenders(said, "x.py"), [])

    def test_a_test_about_the_merge_is_not_flagged(self):
        """The decoy, and why this is not a count of `upsert` calls. Most of the suite's
        two-upsert tests want the merge — that is what `upsert` is for."""
        for second in ("again, _ = events.upsert(conn, {'key': first.key})",
                       "again, verb = events.upsert(conn, {'title': 'poker'})"):
            tree = ast.parse("class TestX:\n"
                             "    def test_y(self):\n"
                             "        first, _ = events.upsert(conn, {'title': 'poker'})\n"
                             f"        {second}\n"
                             "        self.assertEqual(again.id, first.id)\n")
            self.assertEqual(self._offenders(tree, "x.py"), [], second)


class TestAnOccurrenceOffItsSlotSaysWhichSlotItReplaces(Base):
    """`apply._stands_in_for` derives `instead_of`, and every test that
    exercised the column handed a value straight to `events.upsert` instead. Replacing
    the whole function with `return None` — the model's claim unvalidated and an
    unclaimed stand-in never derived — left all 1443 tests green."""

    def setUp(self):
        super().setUp()
        db.set_today(date(2026, 8, 10))                 # the Monday before the slot
    ...
    def _applied(self, **row):
        ...
        evidence = self.conn.execute(
            "SELECT * FROM archive WHERE id = ?", (aid,)).fetchone()
        bundle = bundle_stage.Bundle(entity="person:Quinn Brooks", items=[evidence])
        fields = {"title": "Tutoring", "kind": "commitment", "series": "tutoring", **row}
        apply_stage.apply_diffs(self.conn, self.cfg, [(bundle, {"events": [fields]})],
                                written_by="dream:nightly")
        return events.search(self.conn, "Tutoring")[0]

    def test_a_day_beside_the_slot_records_the_slot_it_replaces(self):
        """The moved week. Without this the Wednesday is a second tutoring row and the
        Tuesday it replaced is still owed one."""
        self.assertEqual(self._applied(date="2026-08-12").instead_of, "2026-08-11")

    def test_the_scheduled_day_itself_stands_in_for_nothing(self):
        self.assertIsNone(self._applied(date="2026-08-11").instead_of)

    def test_a_claim_the_rule_agrees_with_is_kept(self):
        row = self._applied(date="2026-08-19", instead_of="2026-08-18")
        self.assertEqual(row.instead_of, "2026-08-18")

    def test_a_claim_the_rule_does_not_recognise_is_re_derived(self):
        """A model may say any date at all. `2026-08-13` is a Thursday this rule never
        lands on, so the claim is dropped and the real slot derived underneath it."""
        row = self._applied(date="2026-08-12", instead_of="2026-08-13")
        self.assertEqual(row.instead_of, "2026-08-11")

    def test_a_series_with_no_rule_derives_nothing(self):
        """The decoy: `_stands_in_for` may only speak for a rule that exists, or every
        loose row carrying a series name would claim a slot."""
        self.assertIsNone(
            apply_stage._stands_in_for(
                self.conn, {"series": "book-club", "date": "2026-08-12"}, None))


class TestTheAnswerToAGatedLineArrivesWithIt(Base):
    """`bundle.add_thread_context` is the deliberate partial undo of the gate: "u going
    tomorrow" passes it, the "yeah" that answers does not, and the association only
    exists if both are in one bundle. Making the function a no-op left the suite green
    — its name appeared only in a docstring."""
    ...
    def _line(self, ext, text, *, hour, minute, gated):
        ...
        if gated:
            archive.spool_add(self.conn, aid, gate.bundle_entity(
                "Quinn Brooks", "+19175550013", "imessage"))
        self.conn.commit()
        return aid

    def test_the_ungated_reply_comes_with_the_line_it_answers(self):
        self._line("ask", "u going tomorrow", hour=20, minute=0, gated=True)
        self._line("reply", "yeah", hour=20, minute=5, gated=False)
        bundle = bundle_stage.build(self.conn)[0]
        self.assertIn("yeah", [row["text"] for row in bundle.items])

    def test_a_line_outside_the_window_stays_out(self):
        """`CONTEXT_MINUTES` is the whole rule. Without the bound this is not "restore
        the referent", it is "ungate the thread"."""
        self._line("ask", "u going tomorrow", hour=20, minute=0, gated=True)
        self._line("far", "unrelated small talk", hour=23, minute=30, gated=False)
        bundle = bundle_stage.build(self.conn)[0]
        self.assertNotIn("unrelated small talk", [row["text"] for row in bundle.items])


class TestTheBriefNamesTheRuleAndNotEveryInstance(Base):
    """`brief._recurring_block` renders `## Regularly`. Nothing under `tests/` mentioned
    the block or the function, and returning `""` from it left the suite green — the
    rule-rather-than-instances half of the brief was rendered by nobody's assertion."""
    ...
    def test_an_active_rule_is_named_with_its_cadence(self):
        series.roll_forward(self.conn, slug="tutoring")
        text = brief.render(self.conn, self.cfg)
        self.assertIn("## Regularly", text)
        self.assertIn(series.get(self.conn, "tutoring").phrase, text)

    def test_a_rule_with_nothing_on_the_calendar_is_not_announced(self):
        """The block reports the rule *and* its next occurrence. A rule whose occurrence
        has not been materialised has nothing to point at, and announcing it would be
        the brief naming a day with no row behind it."""
        text = brief.render(self.conn, self.cfg)
        self.assertNotIn("## Regularly", text)

    def test_a_cancelled_slot_is_not_called_the_next_appointment(self):
        series.roll_forward(self.conn, slug="tutoring")
        first = events.by_series(self.conn, "tutoring")[0]
        events.upsert(
            self.conn,
            {"key": first.key, "date": first.date, "status": "declined",
             "instead_of": first.date},
            written_by="live", match=False,
        )
        series.roll_forward(self.conn, slug="tutoring")
        text = brief.render(self.conn, self.cfg)
        self.assertIn("next Tue 18 Aug", text)
        self.assertNotIn("next Tue 11 Aug", text)


class TestACrashedPassLooksExactlyLikeOneStillRunning(Base):
    """`run.dream` wrote `_finish` on every path it thought about and none it did not,
    so an uncaught exception left a `runs` row with `finished_at` and `error` both NULL
    — which is what a pass in progress looks like. Runs 4 and 24 in the live store are
    both in that state, three weeks apart, and nothing said so either time."""
    ...
    def _runs(self):
        return self.conn.execute(
            "SELECT id, finished_at, error FROM runs ORDER BY id").fetchall()

    def test_an_uncaught_exception_still_closes_the_row(self):
        from memcal.dream import run as dream_run
        self._spooled()
        with mock.patch.object(dream_run.llm, "client_for",
                               side_effect=RuntimeError("the disk went away")):
            with self.assertRaises(RuntimeError):
                dream_run.dream(self.conn, self.cfg, mode="nightly")
        row = self._runs()[-1]
        self.assertIsNotNone(row["finished_at"], "the row is still open")
        self.assertIn("the disk went away", row["error"])

    def test_the_exception_is_not_swallowed(self):
        """Recording the end must not turn a crash into a quiet return, or the next
        reader learns the pass succeeded."""
        from memcal.dream import run as dream_run
        self._spooled()
        with mock.patch.object(dream_run.llm, "client_for",
    ...
    def test_a_row_an_older_pass_left_open_is_named(self):
        """The two already in the store. Only one pass runs at a time, so a row older
        than the one just opened and still showing neither a finish nor an error is a
        pass that died. `finished_at` stays NULL because it genuinely never finished —
        the error is what makes it tell apart from one still running."""
        from memcal.dream import run as dream_run
        self.conn.execute(
            "INSERT INTO runs(started_at, mode, model, bundles, items)"
            " VALUES(?,?,?,?,?)", (db.now(), "nightly", "test", 9, 64))
        self.conn.commit()
        abandoned = self.conn.execute(
            "SELECT id FROM runs ORDER BY id DESC LIMIT 1").fetchone()["id"]

        dream_run.dream(self.conn, self.cfg, mode="nightly")   # no spool: nothing new

        row = self.conn.execute("SELECT finished_at, error FROM runs WHERE id = ?",
                                (abandoned,)).fetchone()
        self.assertIsNone(row["finished_at"])
        self.assertIn("abandoned", row["error"])

    def test_a_run_still_in_flight_is_not_declared_dead(self):
        """The decoy: only *older* rows are marked, so the pass doing the marking does
        not name itself."""
        from memcal.dream import run as dream_run
        dream_run.dream(self.conn, self.cfg, mode="nightly")
        rows = self._runs()
        self.assertTrue(rows, "no run was opened, so this proves nothing")
        for row in rows:
            self.assertNotIn("abandoned", row["error"] or "")
...
class TestTheUnattendedPathNeverHeardOnRetry(Base):
    ...
    def test_the_cli_prints_the_pass_as_it_happens(self):
        ...
        The pass emits all of it and the web UI has always drawn all of it; the CLI
        printed only `waiting`, so a forty-minute run looked like a hang.
        """
        out = io.StringIO()
        show = cli._dream_progress()
        with contextlib.redirect_stdout(out):
            show("stage", {"stage": "model", "state": "waiting",
                           "note": "capacity; waiting 300s (600s so far)"})
            show("stage", {"stage": "propose", "state": "running",
                           "note": "reading 12 bundles"})
            show("stage", {"stage": "apply", "state": "done", "note": "7 writes"})
            show("propose_wave", {"kind": "main", "requests": 3, "bundles": 12})
            show("propose_request", {"ok": True, "label": "Aaron", "done": 4, "total": 12})
            show("propose_request", {"ok": False, "label": "Riley", "done": 4,
                                     "total": 12, "error": "timed out"})
        printed = out.getvalue()
        for fragment in ("capacity; waiting 300s", "reading 12 bundles", "7 writes",
                         "3 request(s) in flight", "[4/12] ok  Aaron",
                         "failed Riley — timed out"):
            self.assertIn(fragment, printed)

    def test_a_re_send_says_it_was_a_re_send(self):
        """A split or second-look wave must not read as bundles nobody planned for."""
        out = io.StringIO()
        show = cli._dream_progress()
        with contextlib.redirect_stdout(out):
            show("propose_wave", {"kind": "second-look", "requests": 1, "bundles": 5})
        self.assertIn("propose (second-look)", out.getvalue())
...
class TestTheCallsThatFailedNeverReachedDisk(Base):
    ...
    def test_a_truncated_reply_leaves_the_record_a_success_leaves(self):
        ...
        self.cfg.pack_bundles = 1
        _good, errors, _notes = propose_stage.propose_all(
    ...
    def test_a_quota_wall_hit_while_splitting_stops_the_pass(self):
        """The check ran once, after the first wave, and the wall is rarely there yet.

        An account that ran dry during the split retries — or during the second look —
        kept firing requests into it for the rest of the pass, which is the whole thing
        `QuotaExhausted` was introduced to stop.
        """
        from memcal import llm
...
class TestStandingRowsExistBeforeDreamReadsCancellations(Base):
    """A recurring appointment must be in current state before its email is proposed."""

    def test_the_next_occurrence_is_materialized_before_bundles_are_built(self):
        db.set_today(date(2026, 9, 8))
        series.upsert(
            self.conn,
            {"slug": "piano-lesson", "title": "Piano lesson", "cadence": "weekly",
             "weekday": 1, "time": "13:00", "effective_on": "2026-09-01"},
            written_by="live",
        )
        visible_at_bundle_time = []

        def build(conn, **_kwargs):
            visible_at_bundle_time.extend(events.by_series(conn, "piano-lesson"))
            return []

        with mock.patch.object(bundle_stage, "build", side_effect=build):
            result = dream_run.dream(self.conn, self.cfg, dry_run=True)

        self.assertTrue(result.nothing_new)
        self.assertEqual(len(visible_at_bundle_time), 1)
        self.assertEqual(visible_at_bundle_time[0].date, "2026-09-08")
