...
class TestASourceThatFilledItsPageSaysSo(unittest.TestCase):
    ...
    def _chat_db(self, count: int) -> Path:
        """A chat.db with `count` recent lines in one conversation."""
        path = Path(self.tmp.name) / "chat.db"
        ...
        src.execute("INSERT INTO handle VALUES(1, '+19175550001')")
        src.execute("INSERT INTO chat VALUES(1, '+19175550001', NULL)")
        src.execute("INSERT INTO chat_handle_join VALUES(1, 1)")
        stamp = int((db.now_dt() - timedelta(days=1)).timestamp()
                    - datetime(2001, 1, 1, tzinfo=timezone.utc).timestamp()) * 10 ** 9
        for n in range(1, count + 1):
            src.execute("INSERT INTO message VALUES(?,?,?,NULL,?,0,1)",
                        (n, f"guid-{n}", f"are we still on for dinner {n}?", stamp))
            src.execute("INSERT INTO chat_message_join VALUES(1, ?)", (n,))
        src.commit()
        src.close()
        return path

    def test_a_full_page_is_reported_as_more_waiting(self):
        chat_db = self._chat_db(12)
    ...
    def test_a_source_that_ran_dry_does_not_ask_for_another_round(self):
        ...
        pass would run the round cap and charge for it."""
        chat_db = self._chat_db(3)
        report = imessage.ingest(self.conn, limit=50, db_path=chat_db)
    ...
    def test_every_source_report_can_say_it_is_behind(self):
        ...
        and a report class missing one of them answers 0 into a column that means
        something else."""
        from memcal import sources as sources_pkg
        sources_pkg.discover() if hasattr(sources_pkg, "discover") else None
        registered = list(getattr(sources_pkg, "_registry", {}).values())
        self.assertTrue(registered, "no sources registered; this proves nothing")
        for module in {type(source).__module__ for source in registered}:
            report_cls = getattr(sys.modules[module], "IngestReport", None)
            if report_cls is None:
                continue                        # uses base.IngestReport directly
            self.assertTrue(issubclass(report_cls, base.IngestReport), module)
            for field_name in ("more", "muted", "too_old"):
                self.assertTrue(hasattr(report_cls(), field_name),
                                f"{module}.IngestReport has no {field_name}")
