...
class TestAShortenedNameDoesNotOpenASecondPage(Base):
    """`exists()` answers about the slug, so a page under a fuller or shorter form of
    the same name reads as absent."""

    def test_a_diminutive_finds_the_page_standing_under_the_full_name(self):
        wiki.set_slot(self.cfg.wiki_dir, "cameron-ortiz", "relationship", "sibling")
        self.assertFalse(wiki.exists(self.cfg.wiki_dir, "cam-ortiz"))
        self.assertEqual(wiki.near_pages(self.cfg.wiki_dir, "cam-ortiz"),
                         ["cameron-ortiz"])

    def test_the_match_is_symmetric(self):
        wiki.set_slot(self.cfg.wiki_dir, "cam-ortiz", "relationship", "sibling")
        self.assertEqual(wiki.near_pages(self.cfg.wiki_dir, "cameron-ortiz"),
                         ["cam-ortiz"])

    def test_a_shared_surname_alone_is_not_a_match(self):
        wiki.set_slot(self.cfg.wiki_dir, "cameron-ortiz", "relationship", "sibling")
        self.assertEqual(wiki.near_pages(self.cfg.wiki_dir, "jordan-ortiz"), [])

    def test_a_shared_given_name_alone_is_not_a_match(self):
        wiki.set_slot(self.cfg.wiki_dir, "cameron-ortiz", "relationship", "sibling")
        self.assertEqual(wiki.near_pages(self.cfg.wiki_dir, "cameron-vale"), [])

    def test_three_shared_initial_letters_are_not_a_diminutive(self):
        wiki.set_slot(self.cfg.wiki_dir, "cameron-ortiz", "relationship", "sibling")
        self.assertEqual(wiki.near_pages(self.cfg.wiki_dir, "camila-ortiz"), [])

    def test_a_single_name_offers_nothing(self):
        wiki.set_slot(self.cfg.wiki_dir, "cameron-ortiz", "relationship", "sibling")
        self.assertEqual(wiki.near_pages(self.cfg.wiki_dir, "cameron"), [])
