...
class TestGroupMeRosterNamingQueue(unittest.TestCase):
    """Group detail rosters populate the unresolved-handle queue."""
    ...
    def _ingest(self, *, members):
        ...
        self.conn.commit()
        summary = {"id": "Ravers", "name": "Ravers",
                   "messages": {"last_message_id": "m1",
                                "last_message_created_at": int(db.now_dt().timestamp())}}
        detail = {"id": "Ravers", "name": "Ravers", "members": members}
        ...
        class Client:
            ...
            def groups(self):
                return [summary]            # memberships omitted, as production asks
    ...
    def test_ingest_backfills_cached_rosters_once(self):
        ...
        self.conn.execute(
            "INSERT INTO groupme_group_profile_sync(group_id, fetched_at) VALUES(?, ?)",
            ("Ravers", db.now()))
        self.conn.commit()
    ...
    def test_empty_roster_records_nothing(self):
        self.assertEqual(0, threads.record_members(
            self.conn, "groupme", "Ravers", [], review_unknown=True))
        self.assertEqual(1, threads.record_members(
            self.conn, "groupme", "Ravers", [("groupme:42", "Q-Money")],
            review_unknown=True))

    def test_ingest_does_not_read_a_roster_off_the_listing(self):
        """The membership-omitting group listing is not treated as a roster."""
        source = Path(groupme.__file__).read_text(encoding="utf-8")
        ...
        called = {ast.unparse(node.func).rsplit(".", 1)[-1]
                  for node in ast.walk(ingest) if isinstance(node, ast.Call)}
        self.assertNotIn("record_members", called)
        self.assertNotIn("group_members", called)
        self.assertIn('omit="memberships"', source,
                      "if the listing carries members again, read them here on purpose")
