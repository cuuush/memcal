...
#: to-do closes conversationally, never by inference.
...
NOT_WORTH_ASKING = re.compile(
    ...
    # Asking them to deduplicate the calendar. "Were the three August 7 Solstice
...
#: Two places are the same place when either name contains the other, so "Solstice" and
#: "Solstice, Rivertown PA" do not read as a conflict. Deliberately generous: the cost of
...
# a stale question sits in the brief taking a slot from a live one. "Alex Rivera sent
...
def ask(conn: sqlite3.Connection, text: str, *, key: str | None = None,
    ...
    # A wake is code's fallback when new traffic makes an obligation actionable. Dream
    # may already have asked about that exact to-do in better words during the same
    # pass. Prefer its question to a second mechanical "Still open?" row; scope this to
    # the reserved wake key so separate human questions about one obligation still fit.
...
def _title_word_counts(conn: sqlite3.Connection) -> dict[str, int]:
    ...
    "solstice" is in both "Solstice festival" and "Breakfast at Solstice", so it does
