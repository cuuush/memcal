...
GROUPME_NAMING_GENERATION = "1"


def _backfill_naming_queue(conn: sqlite3.Connection) -> int:
    """Queue unresolved handles from rosters cached by older versions."""
    key = "groupme.naming_generation"
    if db.get_meta(conn, key, "") == GROUPME_NAMING_GENERATION:
        return 0
    queued = 0
    rows = conn.execute(
    ...
    for row in rows:
        before = conn.execute(
            "SELECT 1 FROM unresolved WHERE handle = ?", (row["handle"],)).fetchone()
        identity.note_unresolved(conn, row["handle"], "groupme", row["seen_name"])
        after = conn.execute(
            "SELECT 1 FROM unresolved WHERE handle = ?", (row["handle"],)).fetchone()
        queued += int(before is None and after is not None)
    db.set_meta(conn, key, GROUPME_NAMING_GENERATION)
    return queued
...
def _cache_group_profiles(conn: sqlite3.Connection, summary: dict, detail: dict,
    ...
    if roster:
        # Only group detail carries the roster used for naming review.
        threads.record_members(conn, "groupme", thread, roster, review_unknown=True)
...
def message_text(message: dict) -> str:
    """Return visible message text or an attachment summary."""
    text = textclean.spoken_text((message.get("text") or "").strip())
    if text:
...
def ingest(conn: sqlite3.Connection, cfg: Config, *, limit: int = 500,
    ...
    queued = _backfill_naming_queue(conn)
    if queued:
        report.notes.append(f"queued {queued} cached participant(s) for naming")
    ...
    try:
        ...
        report.error = str(exc)
        return report
    # Record listing metadata; detail requests supply rosters later.
    pending = []
    initial_skipped = 0
    for group in groups:
        ...
        # The listing omits memberships; group detail refreshes rosters later.
    ...
    # Refresh rosters for groups with gated messages.
    if not stopped:
        if progress:
