...
_HOLIDAY_CALENDAR = re.compile(r"\bholidays?\b", re.IGNORECASE)
...
def _subscribed_holiday(item: dict) -> bool:
    """Whether this is a system holiday feed rather than the user's own plan."""
    return (not bool(item.get("writable"))
            and bool(_HOLIDAY_CALENDAR.search(str(item.get("calendar_name") or ""))))
...
def _exact_same_day_event(
    ...
    `find_match`'s absorption tier, so "Solstice" never absorbs into
    "Breakfast at Solstice" and near-duplicates ("deep block"/"light block")
    stay separate rows.
    """
...
def ingest_snapshot(
    ...
    *,
    ...
    stamp = db.now()
    unchanged = ours = ignored_holidays = 0
    ...
    for index, raw in enumerate(items, 1):
        ...
        if common is None:
            ...
            continue
        if _subscribed_holiday(item):
            # A subscribed holiday is reference information already available in
            # Calendar.app, not an invitation or a plan. Remove any row an older build
            # created for it, but only when that row came solely from this calendar.
            ...
            tracked = conn.execute(
                "SELECT identity, event_key FROM calendar_items"
                " WHERE identity = ? OR (? <> '' AND event_uid = ?)",
                (identity, uid, uid),
            ).fetchall()
            for old in tracked:
                conn.execute("DELETE FROM calendar_items WHERE identity = ?",
                             (old["identity"],))
                remaining = conn.execute(
                    "SELECT 1 FROM calendar_items WHERE event_key = ? LIMIT 1",
                    (old["event_key"],),
                ).fetchone()
                event = events.get(conn, old["event_key"])
                if (remaining is None and event is not None
                        and str(event.origin or "").startswith("ical:subscribed:")):
                    conn.execute(
                        "DELETE FROM provenance WHERE kind = 'event' AND ref = ?",
                        (event.key,),
                    )
                    conn.execute(
                        "DELETE FROM evidence WHERE kind = 'event' AND ref = ?",
                        (event.key,),
                    )
    ...
    if ignored_holidays:
        report.notes.append(
            f"ignored {ignored_holidays} event(s) from subscribed holiday calendars")
    conn.commit()
    return report
...
def publish_reminder(cfg, todo, *, runner=subprocess.run) -> dict:
    ...
    outside the process, so it remains disabled by default.
    """
...
def publish_pending(conn: sqlite3.Connection, cfg, *, keys=None,
    ...
    for row in rows:
        ...
        if event.series and not event.instead_of:
            if conn.execute(
                    "SELECT 1 FROM calendar_items WHERE event_key = ? AND published = 1",
                    (series_key(event.series),)).fetchone():
                continue
            # Never publish a scheduled occurrence without its rule.
            log.append(f"{event.key}: belongs to series {event.series!r}, whose rule is "
                       f"not published — not publishing the occurrence on its own")
            continue
        known = conn.execute(
...
class ICalSource(Source):
    ...
    def fetch(self, conn: sqlite3.Connection, cfg, report, limit: int) -> None:
        ...
        # Partiful's calendar file omits host names, while its public event and profile
        # pages expose them. Enrich before revision comparison so existing rows are
        # updated once; failure is non-fatal and bounded inside the provider.
        providers.partiful.enrich_hosts(items, conn=conn, report=report)
        if full:
