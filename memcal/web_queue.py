...
def _item(row: sqlite3.Row, with_whom: dict[tuple, str] | None = None) -> dict:
    ...
    # For mail the subject is the skimmable part. The body is fetched for every message
    # in scope now, so an empty one here means it was never read rather than never
    # wanted — `_body_state` says which.
    body = text
    ...
    return {
        ...
        # Whether the body behind this row was read, shortened, or never fetched. A row
        # that looks complete and is a subject line is the failure this reports.
        "body": _body_state(meta),
    }


def _body_state(meta: dict) -> str:
    if meta.get("body_excluded"):
        # Not a gap. The user said no to this sender, so the body was never opened.
        return "not fetched — sender blocked"
    if meta.get("body_error"):
        return "fetch failed"
    if meta.get("body_truncated"):
        return "shortened"
    if meta.get("body_chars"):
        return "full"
    return "" if "body_chars" in meta else "no body on file"
...
def _filters(*, stream: str = "", verdict: str = "", reason: str = "",
    ...
    # The quiet half of the mailbox, on purpose and by itself. Low-priority mail is read
    # like everything else, later and in bounded batches; this is where a person goes to
    # see what memcal has been quietly getting through, and to disagree with it.
...
def groups(conn: sqlite3.Connection, *, stream: str = "", verdict: str = "",
    ...
    rows = conn.execute(
        f"""SELECT a.stream, {GROUP_KEY} AS key,
                  ...
                  sum(CASE WHEN coalesce(s.priority, 'normal') = 'low'
    ...
    for row in rows:
        ...
        out.append({
            ...
            # Both halves, because "quiet" without "how much of it has actually been
            # read" is the reassuring half of the answer on its own.
            "quiet": row["quiet"] or 0,
