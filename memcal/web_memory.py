...
from . import (archive, brief, calls, db, detail, identity, presentation, textclean, todos,
               trace, wiki)
...
def overview(conn: sqlite3.Connection, cfg: Config, days: int = 14) -> dict:
    ...
    return {
        ...
        # Split by priority, because "1,400 waiting" and "1,340 of them are the quiet
        # backlog memcal is working through in batches" are different situations and
        # only one of them is a problem. A backlog that is never stated is a backlog
        # that silently grows.
        "backlog": archive.backlog(conn),
        ...
        "brief_tokens": textclean.estimate_tokens(text),
        "brief_cap": cfg.brief_token_cap,
        "prefix_tokens": textclean.estimate_tokens(propose_stage.build_prefix(conn, cfg)),
        "spend": round(spend, 4),
...
#: Compatibility export; row detail is assembled in `detail`.
_wiki_link = detail._wiki_link
