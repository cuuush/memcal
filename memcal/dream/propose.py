...
constraint makes deduplication automatic.
"""
...
from .. import (actions, calls, db, events, identity, llm, questions, textclean,
                threads, todos, trace, wiki)
...
#: Which lines support *each field you changed*, rather than the row as a whole.
#:
#: Write precedence is decided per field and on when the supporting line was said, so a
#: single pooled citation hands the freshest line's authority to every other field in the
#: diff — the newest thing anyone said about *when* silently re-deciding *where*. Code
#: cannot recover this by matching values against the text: a line may quote an old
#: arrangement, negate it, or move something without repeating a single literal from the
#: row. Only the reader knows which line it read.
_FIELD_CITES = {
    "type": ["object", "null"],
    "additionalProperties": {"type": "array", "items": {"type": "string"}},
    "description": ("per changed field, the L-tags supporting *that field*, e.g. "
                    "{\"time\": [\"L7\"], \"location\": [\"L3\"]}. Cite the line that "
                    "states the new value, not one quoting or denying the old one. A "
                    "field you cannot point at a line for may not overrule a value "
                    "already settled"),
}
...
#:
#: This is a typed diff against a key, not a verb or tool call. `series.upsert`
#: deterministically applies the rule to stored rows.
...
def build_bundle_block(cfg: Config, bundle: Bundle,
    ...
    if conn is not None:
        ...
        done = build_completed_actions(conn, bundle)
        if done:
            context.append(done)
    ...
    if missing:
        ...
        # "No page yet" is a stronger claim than `exists()` can support: true of the
        # slug, false of the person whenever a page stands under a fuller or shorter
        # form of the same name. Stated flatly it produces duplicate person pages.
        near = [(person, page) for person in missing
                for page in wiki.near_pages(cfg.wiki_dir, db.slugify(person))]
        if near:
            context.append(
                "MAY ALREADY HAVE A PAGE: "
                + "; ".join(f"{person} → {page}" for person, page in near)
                + " — if the bundle shows they are one person, write the fact on the"
                  " existing page and set `alias` instead of opening a second one.")
...
def _amendable_groups(conn: sqlite3.Connection, bundle: Bundle
    ...
    same, related, _nominated, _cut = _candidates(conn, bundle)
    return same, related


def _candidates(conn: sqlite3.Connection, bundle: Bundle
                ) -> tuple[list[events.Event], list[events.Event],
                           list[events.Event], int]:
    """Every row this conversation could be about, in three tiers of confidence."""
...
def build_completed_actions(conn: sqlite3.Connection, bundle: Bundle) -> str:
    """What the assistant already did, beside the statement that caused it.

    Two ways in, because the two failures are different. By *line*: this very sentence
    has already been carried out, so re-extracting a plan from it writes a second row.
    By *target*: a row this conversation is about was moved at lunchtime, so a fragment
    of the older arrangement is not news however confidently it is worded.
    """
    ids = [int(row["id"]) for row in bundle.items
           if "id" in row.keys() and row["id"]]
    same, related = _amendable_groups(conn, bundle)
    seen: dict[str, actions.Action] = {}
    for action in (actions.for_archive(conn, ids)
                   + actions.for_refs(conn, "event",
                                      [event.key for event in [*same, *related]])):
        seen[action.op_id] = action
    ordered = sorted(seen.values(), key=lambda a: (a.at, a.op_id))
    return actions.render(ordered)
...
def build_open_rows(conn: sqlite3.Connection, bundle: Bundle) -> str:
    ...
    graph_entities = [bundle.entity, *bundle.merged]
    same, related, nominated, cut = _candidates(conn, bundle)
    if not same and not related and not nominated:
        return ""
    ...
    def append_event(event, *, from_here: bool) -> None:
        ...
        if event.hosts:
            bits.append("hosted by " + ", ".join(event.hosts))
    ...
    if nominated:
        # Named as a nomination rather than a link, because that is what it is: nothing
        # connects these rows to this conversation except the words. The decision is the
        # model's, and "cannot tell" is one of the answers — a guess here is a false
        # merge, and a false merge is worse than the duplicate it was avoiding.
        lines.append(
            "POSSIBLY THE SAME PLAN, DIFFERENTLY WORDED. Nothing links these to this"
            " conversation except wording, so decide: same row (return its key), a"
            " different occasion (return your own row with no key), or leave it alone if"
            " you cannot tell from what is here.")
        for event in nominated:
            append_event(event, from_here=False)
    if cut:
        # Said out loud. A truncated candidate list is indistinguishable from a short
        # one, and a model that was never shown the row cannot be said to have decided
        # the plan was new.
        lines.append(f"  ({cut} further row(s) matched on wording and were not shown)")
    return "\n".join(lines)
...
def propose_all(client: CompletionClient, conn: sqlite3.Connection, cfg: Config,
    ...
    belongs on `runs.error`. A recovery — a truncated batch split and re-sent, a skipped
    bundle asked about again — is the pass working. One list conflated them, so a clean
    run reported itself as a failed one.
    """
    ...
    while stranded:
        ...
        notes.append(f"split {len(stranded)} truncated request(s) into {len(halves)} "
                     f"and re-sent")
    ...
    if doubtful:
        notes.append(f"asked again about {len(doubtful)} bundle(s) the gate or event "
                     f"graph read as plans and the model said nothing about "
...
def _resolve_cites(bundle: Bundle, diff: dict) -> None:
    ...
    Validation lives in `Bundle.cite`, which resolves only tags naming a line of *this*
    bundle and silently drops anything else — an index out of range, a tag copied from a
    neighbouring bundle, a hallucinated number. A citation that resolves to nothing is
    therefore indistinguishable from no citation, which is the safe direction: it costs
    ...
    for key in ("events", "todos", "questions"):
        for row in diff.get(key) or []:
            ...
            claimed = row.pop("field_cites", None)
            if isinstance(claimed, dict):
                resolved = {str(field): bundle.cite(tags)
                            for field, tags in claimed.items()
                            if isinstance(tags, (list, tuple))}
                row["field_cite_ids"] = {field: ids for field, ids in resolved.items()
                                         if ids}
