...
from .. import (dates, db, events, identity, pending, questions,
                series as series_mod, todos, trace, wiki)
...
#: Every field a row can assert support for. Ordering is decided per field, so the list
#: is the domain of `_evidence_by_field`.
_DATED_FIELDS = ("date", "until", "time", "location", "title", "note", "status",
                 "participants", "hosts", "kind", "subject", "series")


def _evidence_ts(bundle: Bundle, cited, claims) -> str | None:
    """When the lines behind *this row* were said, not when the bundle last had traffic."""
    by_id = _line_times(bundle)
    picked = [i for i in (cited or ()) if isinstance(i, int) and i in by_id]
    if not picked and claims:
        picked = [i for i in _supporting_lines(bundle, claims) if i in by_id]
    if picked:
        return max(by_id[i] for i in picked)
...
def _evidence_by_field(bundle: Bundle, cited, row: dict) -> dict[str, str] | None:
    ...
    one quoting the old arrangement, one denying it, and misses every update phrased
    without the literal. So the model cites per field, `_resolve_cites` validates those
    references against this bundle, and ordering is applied here. An uncited field gets
    ...
    if not by_id:
        ...
        # row-level contract rather than being read as a reader declining to support
        # anything.
        return None
    occasion = _evidence_ts(bundle, cited, row.get("title"))
    # Presence, not emptiness. A reader that cited per field and had nothing to cite for
    # the place has said something — "I did not read a line stating it" — and that is the
    ...
    for name in _DATED_FIELDS:
        if row.get(name) in (None, "", []):
            continue
        lines = [i for i in (claimed.get(name) or ()) if i in by_id]
        if lines:
            out[name] = max(by_id[i] for i in lines)
    return out
...
def _readable(candidate: str) -> str:
    ...
    BGC board-game gathering" and "+100000000000200 asked: …".
...
def _named_only_by_thread(bundle, row) -> bool:
    ...
    # and its first word is usually the whole name — "Chili's", "Solstice", "Poker".
...
def _apply_event(conn: sqlite3.Connection, row: dict, *, source: str, written_by: str,
    ...
    if not isinstance(row, dict):
        return None
    # A pending observation is one statement, not a set of fields, so it records the one
    # moment it was said — the latest of whatever supported the diff it came out of.
    said_at = (max((str(v) for v in evidence_ts.values() if v), default=None)
               if isinstance(evidence_ts, dict) else evidence_ts)
    title = _clean(row.get("title"))
    date_value = _clean(row.get("date"))
    status_said = _clean(row.get("status")) or ""
    if not title or not date_value:
        # A cancellation that names neither. Dropping it loses a real statement about
        # the user's week; filling in a title and a date writes a cancelled plan that
        # never existed. Keep the observation, guess nothing, and try again when more
        # evidence arrives — see `memcal/pending.py`.
        if status_said == "declined":
            said = title or _clean(row.get("note")) or _clean(row.get("location"))
            if said:
    ...
    if status == "declined" and not key and not row.get("_allow_declined_insert") \
        ...
        pending.note(conn, kind="cancellation",
                     ...
                     # and stated so the contract is visible at the call site.
                     target_key="", commit=False)
