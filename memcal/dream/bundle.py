...
def stats(bundles: list[Bundle]) -> str:
    """Summarize spooled lines and restored thread context."""
    items = sum(len(b.items) for b in bundles)
    spooled = sum(len(b.spool_ids) for b in bundles)
    chars = sum(len(r["text"] or "") for b in bundles for r in b.items)
    restored = max(0, items - spooled)
    context = f" ({restored} restored as context)" if restored else ""
    return (f"{len(bundles)} bundles · {items} items{context} · "
            f"~{chars // 4} tokens of traffic")
