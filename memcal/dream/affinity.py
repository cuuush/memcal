...
#: checkable by a human; "2026-08-02 · garden, harborview" is.
...
def group(bundles: list, *, max_bundles: int, max_tokens: int, cost,
    ...
    for (left, right), _hits in sorted(scores.items(), key=lambda kv: -kv[1]):
        if left in placed or right in placed:
            continue
        # Leave both bundles available when the seed pair exceeds the request budget.
        if costs[left] + costs[right] > max_tokens:
            continue
        members = [left, right]
