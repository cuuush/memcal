"""Stage 3: reconcile mentions into one event row."""
...
#:     population to mean anything; resolve sees one run's proposed rows. At that size the
...
def resolve_all(client: CompletionClient, cfg: Config, proposals: list,
                *, conn: sqlite3.Connection | None = None,
                run_id: int | None = None) -> tuple[list, list[str]]:
    """Collapse cross-bundle duplicate events. Returns (proposals, log).

    `proposals` is propose's own output, `[(bundle, diff, generation_id), ...]`, and the
    same shape comes back: the surviving row is left on the earliest-dated fragment's
    bundle, and the others have it removed. Everything that is not an event — to-dos,
    wiki slots, questions — passes through untouched.
    """
    ...
    for group in cluster(mentions, cfg):
        ...
        try:
            reply = client.complete(
                ...
                schema=SCHEMA, schema_name="memcal_resolve", max_tokens=1200)
            if conn is not None:
                trace.record(conn, run_id=run_id, stage="resolve",
            ...
            log.append(f"resolve failed for {sources}: {exc} — merged on agreement only")
            merged = _merge_locally(group)
            _corroborate(merged, voices)
            _collapse(group, merged)
            continue
        ...
        log.append(f"resolved {len(group)} mentions of {merged['title']!r} ({sources})"
                   + (f" — {why}" if why else ""))
    return proposals, log
