...
class Config:
    ...
    # Completion transport. The CLI providers use the caller's existing Claude Code,
    ...
    # Total and per-entity ceilings for spooled lines. Restored thread context is extra.
...
def load(home: str | os.PathLike[str] | None = None) -> Config:
    ...
    # Outward-write settings are scoped to the selected store.
    store_env = load_env(home_path / ".env")
    ...
    #: Settings that can write outside this process.
    for name, attr in (
        ("MEMCAL_PUBLISH_CALENDAR", "publish_calendar"),
        ("MEMCAL_PUBLISH_REMINDERS", "publish_reminders"),
    ):
        raw = store_env.get(name) or os.environ.get(name)
        if raw:
