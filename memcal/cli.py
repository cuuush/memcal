...
def cmd_brief(args) -> int:
    ...
    if args.tokens:
        print(f"\n[~{textclean.estimate_tokens(text)} tokens / cap {cfg.brief_token_cap}]",
              file=sys.stderr)
    return 0
...
def cmd_who(args) -> int:
    ...
    if getattr(args, "confirm", None):
        said = whois.confirm(conn, args.confirm)
        print(f"confirmed: {said}" if said
              else f"#{args.confirm} is not open, has no candidate to confirm, or is "
                   f"a merge the store refuses")
        return 0 if said else 1
    if getattr(args, "split", None):
        put_back = whois.split(conn, args.split)
        print(f"split: {put_back} is its own person again" if put_back
              else f"no assumption #{args.split} to split")
    ...
    if getattr(args, "resolve", False):
        ...
        try:
            found = whois.resolve(client, conn, cfg)
        except LLMError as exc:
            print(f"error: {exc}")
            return 1
        lines = whois.apply(conn, found)
        for line in lines:
            print(f"  {line}")
        if not lines:
            print("  nothing it was willing to conclude")
    ...
    if open_assumptions:
        ...
        print("  memcal who --split <n> to undo one; --confirm <n> to stop asking")
    ...
    if doubts:
        ...
        print("  memcal who --confirm <n> to act on one; --split <n> to say no")
        for row in doubts:
            guess = row["keep"] or "?"
    ...
    print("\n  memcal who <handle> \"<name>\"   name one"
          ...
          "\n  memcal who --resolve            one model call over all of it at once"
...
def cmd_mail(args) -> int:
    """Mail bodies memcal never read, and — on request — a run at recovering them."""
    cfg, conn = open_ctx(args)
    from .sources import proton                                      # noqa: PLC0415

    if args.backfill:
        # Dry by default and stated plainly. This one talks to the user's real mailbox,
        # so "what would it do" has to be the answer you get by accident.
        report = proton.backfill_bodies(
            conn, cfg, limit=args.limit, dry_run=not args.apply,
            progress=(lambda note: print(f"  {note}")) if args.apply else None)
        print(f"{report['candidates']} archived mail row(s) have no body on file; "
              f"this run covers {report['attempted']}")
        if report["dry_run"]:
            print("dry run — nothing was fetched. Add --apply to run it for real; it "
                  "updates each row in place, so a second run cannot duplicate mail.")
        else:
    ...
    waiting = archive.backlog(conn)
    print(f"queued and unread: {waiting.get('total', 0)}")
    for name in ("normal", "low"):
        item = waiting.get(name)
        if item:
            print(f"  {name:7} {item['waiting']:6}   oldest {item['oldest'] or '-'}")
    print(f"mail with no body on file: {len(proton.recoverable(conn))}"
          "   (memcal mail --backfill)")
    return 0
...
def _dream_progress():
    ...
    def show(event: str, data: dict) -> None:
        nonlocal requests
        if event == "stage":
            stage = data.get("stage") or "model"
            state = data.get("state") or ""
            note = data.get("note") or ""
            if state in ("running", "waiting", "failed"):
                mark = {"waiting": "…", "failed": "!"}.get(state, "")
                print(f"  {stage}{mark}: {note}" if note else f"  {stage}{mark}",
                      flush=True)
            elif state == "done" and note:
        ...
        if event == "propose_wave":
            ...
            print(f"  {opening}: {data.get('requests', 0)} request(s) in flight · "
                  f"{data.get('bundles', 0)} bundle(s)", flush=True)
            return
        if event == "propose_request":
            ...
            print(f"    [{done}/{total}] {state} {data.get('label') or 'request'}{tail}",
                  flush=True)
...
def cmd_schedule(args) -> int:
    ...
    if args.action == "due":
        # The first word is the scheduled script's machine-readable verdict.
        due, why = schedule.owed(cfg)
        print(f"{'owed' if due else 'ok'}: {why}")
        return 0
    ...
    print(f"wakes      every {int(st['every']) // 60} min, at login, and on wake — "
          f"a missed pass runs then")
    print(f"last pass  {st['why']}")
    if st["warning"]:
...
def _minutes_since(stamp) -> float:
    """Return minutes since `stamp`, treating invalid stamps as old."""
    try:
        return (db.now_dt() - db.parse_ts(str(stamp))).total_seconds() / 60
...
def doctor_findings(conn: sqlite3.Connection, cfg: Config, *,
    ...
    if loud:
        ...
        add("Store", "unresolved handles", WARN,
            ...
            fix="memcal who --resolve   # name them in one pass; every row they touch joins up")
    else:
    ...
    elif last and not last["finished_at"] and _minutes_since(last["started_at"]) > 30:
        # An old unfinished row indicates an interrupted collection.
        add("Sources", "last collection", FAIL,
            f"{_ago(last['started_at'])}: started and never finished — the pass died "
            f"before it could record why",
            fix="memcal ingest --stale")
    elif last:
    ...
    if ledger_error:
        add("Extraction", "cost ledger", FAIL, str(ledger_error)[:90],
            fix="the generations INSERT in trace.record has drifted from schema.sql")
    else:
        runs_cost = float(conn.execute(
            "SELECT coalesce(sum(cost_usd), 0) AS c FROM runs").fetchone()["c"] or 0)
        gen_cost = float(conn.execute(
            "SELECT coalesce(sum(cost_usd), 0) AS c FROM generations").fetchone()["c"] or 0)
        lost = runs_cost - gen_cost
        add("Extraction", "cost ledger", WARN if lost > 0.005 else OK,
            f"runs ${runs_cost:.4f} vs generations ${gen_cost:.4f}",
            fix="memcal trace" if lost > 0.005 else "")
    ...
    tokens, cap = textclean.estimate_tokens(text), cfg.brief_token_cap
    ...
    add("Extraction", "shared prefix", INFO,
        f"~{textclean.estimate_tokens(propose_stage.build_prefix(conn, cfg))} tokens, cached per run")
    ...
    if not state["installed"]:
        ...
        bad = bool(state["warning"]) or not state["loaded"]
        add("Schedule", "nightly", WARN if bad else OK,
            state["warning"]
            or (f"{state['label']} daily at {when}, next {state['next']}"
                if state["loaded"] else f"{state['label']} is installed but not loaded"),
            fix="memcal schedule install" if bad else "")
    add("Schedule", "last pass", OK if state["installed"] else WARN,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    s.add_argument("--resolve", action="store_true",
                   help="one model call over the whole picture: name the queue, merge "
                        "name variants, drop what is not a person")
    s.add_argument("--confirm", type=int, metavar="N",
                   help="yes: stop listing merge N, or act on doubt N")
    s.add_argument("--split", type=int, metavar="N",
                   help="no: undo merge N, restoring the handles it moved")
    ...
    s = sub.add_parser("mail", help="mail backlog, and recovering bodies memcal never read")
    s.add_argument("--backfill", action="store_true",
                   help="re-fetch bodies for archived mail that has none")
    s.add_argument("--apply", action="store_true",
                   help="actually fetch. Without it --backfill only reports")
    s.add_argument("--limit", type=int, default=200,
                   help="how many rows this run may cover; the rest wait for the next")
    ...
    s.add_argument("action", nargs="?", default="status",
                   choices=["status", "install", "uninstall", "run", "due"],
                   help="status, install, uninstall, run it now, or ask whether a pass "
                        "is owed")
