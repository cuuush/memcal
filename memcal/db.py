...
#: Bumped when archived-body visibility rules change.
SPOKEN_GENERATION = "1"


def _retire_unspoken_rows(conn: sqlite3.Connection) -> int:
    """Retire archived bodies with no visible content."""
    if get_meta(conn, "archive.spoken_generation", "") == SPOKEN_GENERATION:
        return 0
    retired = 0
    for row in conn.execute(
            "SELECT id, text FROM archive WHERE text IS NOT NULL AND text <> ''"
    ).fetchall():
        if textclean.spoken_text(row["text"]):
            continue
        conn.execute(
        ...
        retired += 1
    set_meta(conn, "archive.spoken_generation", SPOKEN_GENERATION)
    return retired
